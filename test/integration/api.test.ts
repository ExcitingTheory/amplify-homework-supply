...
describe("A. Authentication & Authorization Tests", () => {
  describe("A1. User Authentication", () => {
    ...
    test("Session persistence across page refreshes", async () => {
      ...
      expect(session1.tokens?.accessToken.toString()).toBe(
        session2.tokens?.accessToken.toString(),
      );
    });
  ...
  describe("A2. Authorization & Group Access", () => {
    ...
    test("Learner can view published units", async () => {
      ...
      const { data, errors } = await client.models.Unit.get({
        id: created!.id,
      });
    ...
    test("Admin has full access to all resources", async () => {
      ...
      // Admin can delete
      const { data: deleted } = await safeDelete(
        client.models.Unit,
        created!.id,
      );
  ...
  describe("A3. S3 File Access", () => {
    ...
    test("Public files accessible to all authenticated users", async () => {
      ...
      await signInAs("student1");
      const { data, errors } = await client.models.File.get({
        id: created!.id,
      });
    ...
    test("Protected files only accessible to owner", async () => {
      ...
      await signInAs("student1");
      const { data: accessed } = await client.models.File.get({
        id: created!.id,
      });
...
describe("B. GraphQL API Tests (Gen 2)", () => {
  describe("B1. Unit CRUD Operations", () => {
    ...
    test("Delete unit", async () => {
      ...
      // Delete
      const { data: deleted, errors } = await safeDelete(
        client.models.Unit,
        created!.id,
      );
      ...
      // Verify deletion
      const { data: notFound } = await client.models.Unit.get({
        id: created!.id,
      });
  ...
  describe("B2. Assignment CRUD Operations", () => {
    ...
    test("Update assignment due date", async () => {
      ...
      const { data: updated, errors } = await safeUpdate(
    ...
    test("Delete assignment", async () => {
      ...
      // Verify deletion
      const { data: notFound } = await client.models.Assignment.get({
        id: created!.id,
      });
      expect(notFound).toBeNull();
    });
  ...
  describe("B3. Grade CRUD Operations", () => {
    ...
    test("Complete grade (mark as complete)", async () => {
      ...
      // Mark as complete
      const { data: completed, errors } = await safeUpdate(
        client.models.Grade,
        {
          id: created!.id,
          percentComplete: 100,
  ...
  describe("B4. Section CRUD Operations", () => {
    ...
    test("Delete section", async () => {
      ...
      // Verify deletion
      const { data: notFound } = await client.models.Section.get({
        id: created!.id,
      });
      expect(notFound).toBeNull();
    });
  ...
  describe("B5. Word/Question/File CRUD", () => {
    ...
    test("Update word with audio files", async () => {
      ...
      expect(updated?.audio).toEqual([
        "public/audio/hello-1.mp3",
        "public/audio/hello-2.mp3",
      ]);
