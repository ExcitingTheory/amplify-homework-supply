...
function walkDir(dir: string): string[] {
  ...
  for (const entry of entries) {
    ...
    if (entry.name === 'node_modules' || entry.name === '.next' || entry.name === 'storybook-static') continue;
...
const ALL_PERSONAS_WITH_ALL: Array<UserPersona | 'all'> = [...ACTIVE_PERSONAS, 'all'];
const DATA_TOUR_SOURCE_DIRS = ['src/components', 'src/stories', 'pages', '.storybook/components', '.storybook/addons'];
...
for (const dir of DATA_TOUR_SOURCE_DIRS) {
  ...
  for (const file of walkDir(absDir)) {
    ...
    const attrMatches = content.match(/data-tour\s*=\s*["']([^"']+)["']/g) || [];
    for (const m of attrMatches) {
      const val = m.match(/["']([^"']+)["']$/);
      if (val) allDataTourValues.add(val[1]);
    }
    // Match 'data-tour': 'value' or "data-tour": "value" (spread pattern)
    const spreadMatches = content.match(/['"]data-tour['"]\s*:\s*['"]([^'"]+)['"]/g) || [];
...
describe('Onboarding System — Data Integrity', () => {
  ...
  it('should have unique spotlight config taskIds', () => {
    ...
    for (const id of configTaskIds) {
      expect(seen.has(id), `Duplicate spotlight config taskId: ${id}`).toBe(false);
      seen.add(id);
    }
  });
  ...
  for (const persona of ACTIVE_PERSONAS) {
    describe(`persona: ${persona}`, () => {
      ...
      it('getTasksByCategory should group tasks correctly', () => {
        ...
        const flatCount = Object.values(grouped).reduce((sum, arr) => sum + arr.length, 0);
