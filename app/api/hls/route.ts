...
async function loadCFCredentials(): Promise<{ privateKey: string; keyPairId: string }> {
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const signedManifestUrl = signCFUrl(manifestUrl, privateKey, keyPairId, manifestExpiresAt);

    const manifestRes = await fetch(signedManifestUrl);
    if (!manifestRes.ok) {
      return NextResponse.json(
        { error: `Failed to fetch manifest: ${manifestRes.status}` },
        { status: manifestRes.status >= 400 && manifestRes.status < 500 ? 404 : 502 },
      );
    }
