...
export default function WordListClient({
  ...
  const [words, setWords] = React.useState<WordRecord[]>(initialWords);
  const [loading, setLoading] = React.useState(false);
  const [stats, setStats] = React.useState<WordHealthStats | null>(() =>
    computeStats(initialWords),
  );
  ...
  async function loadWords() {
    ...
    try {
      ...
      setWords(allWords);
      setStats(computeStats(allWords));
    } catch (err) {
  ...
  function computeStats(wordList: WordRecord[]): WordHealthStats {
                ...
                {filteredWords.slice(0, 200).map((word) => (
                  ...
                  >
                    ...
                    <TableCell>
                      ...
                      >
                        {word.owner ? word.owner.slice(0, 12) + "..." : "—"}
                      </Typography>
                    </TableCell>
                    <TableCell>
