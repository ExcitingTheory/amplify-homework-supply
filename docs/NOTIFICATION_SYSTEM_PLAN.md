# Notification System — Architecture & Implementation Plan
...
Notification: a
  .model({
    ...
    category: NotificationCategory,           // enum: ASSIGNMENT, COLLABORATION, GAMIFICATION, GUILD, SYSTEM
    title: a.string().required(),
    body: a.string(),
    linkPath: a.string(),                     // e.g. "/section/abc123", "/review/xyz"
    linkLabel: a.string(),                    // e.g. "View Assignment", "Join Review"
    referenceId: a.string(),                  // ID of the source entity (assignment, guild, room, etc.)
    referenceType: a.string(),                // "Assignment", "HomeworkRoom", "Guild", "GroupChallenge", etc.
...
const NotificationType = a.enum([
  ...
  "GUILD_POST_NEW",         // New post in your guild
  "GUILD_MEMBER_JOINED",    // New member joined your guild
  "GUILD_METADATA_UPDATED", // Guild name/description changed
  "GUILD_INVITE",           // Invited to join a guild
...
| Guild post | Client-side when post is created | `GUILD_POST_NEW` |
| Guild member joined | Client-side on guild join | `GUILD_MEMBER_JOINED` |
| Guild metadata changed | Client-side on guild update | `GUILD_METADATA_UPDATED` |
...
- **Drawer menu items** (Sections, Units, Guilds, Skills, Leaderboard)
...
- **Guilds**: `unseenByCategory.GUILD` count
...
**Layout**:
- Filter tabs: All | Assignments | Collaboration | Gamification | Guild | System
...
11. Add client-side notification creation for guild posts, member joins, collaboration invites
...
| Client-side notification creation for some events, that are specific to a user's self or a guild or section an instructor is in or chat someone is a member of | Avoids Lambda cold starts for simple create operations; auth rules enforce owner-only writes |
