# Optimistic Concurrency & observeQuery Migration Plan
...
| 8 | `gamificationContext.tsx` | StudentProfile, Guild, Skill, GroupChallenge | 10+ manual onCreate/onUpdate subs | `observeQuery()` + version guards |
...
| **Guild** | Version map, skip if `_version <=` | `guildVersionMap.current = { [id]: _version }` |
...
### Pattern B: Version Map (Word, Question, File, Guild, Skill, etc.)
...
8. **Gamification** — `observeQuery` + version guards for StudentProfile, Guild, Skill, GroupChallenge
