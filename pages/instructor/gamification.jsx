...
import {
  ...
  useCampaign,
  useGuild,
} from "../../src/context/gamificationContext";
...
function GamificationAdmin() {
  ...
  React.useEffect(() => {
    if (router.query.section && router.query.section !== selectedSectionId) {
      setSelectedSectionId(router.query.section);
    }
  ...
  // Load xpConfig and linearLockEnabled from the selected section
  React.useEffect(() => {
    ...
    async (newConfig) => {
      ...
      try {
        ...
        await client.models.Section.update({
          ...
          xpConfig: JSON.stringify(newConfig),
