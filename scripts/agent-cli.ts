 ...
 *   npx agent-cli execute-skill storybook-audit --dry-run
...
} from "../src/utils/agentPermissions";
...
program
  ...
  .requiredOption(
    "--operation <type>",
    "Operation type (read, write, delete, rename, terminal)",
  )
...
program
  ...
  .requiredOption(
    "--operation <type>",
    "Operation type (read, write, delete, rename)",
  )
  ...
  .action(async (options) => {
    try {
      ...
      await permissions.logOperation(
        options.operation,
        options.path,
        "allowed",
        {
  ...
  .action(async (options) => {
    try {
      ...
      logs.forEach((log, index) => {
        ...
        console.log(
          `${chalk.dim(`${index + 1}.`)} ${chalk.bold(log.operation)} ${log.path || ""}`,
        );
...
program
  ...
  .action(() => {
    ...
    console.log(
      "  agent-cli check --operation write --path src/components/NewComponent.tsx\n",
    );

    console.log(chalk.cyan("Check if a terminal command is allowed:"));
    console.log(
      '  agent-cli check --operation terminal --command "npm test"\n',
    );
    ...
    console.log(
      "  agent-cli execute --operation write --path dangerous.txt --dry-run\n",
    );

    console.log(chalk.cyan("Check skill permissions:"));
    console.log(
      "  agent-cli skill --name mock-data-validator --operation create\n",
    );
