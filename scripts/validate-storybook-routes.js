...
// pages.stories.tsx was split into one file per page (pages-index, pages-units, etc.)
// so opening any single story doesn't force Vite to bundle every page (Lexical editor, etc.)
const STORIES_DIR = path.join(__dirname, "../src/stories");
const STORIES_FILE_PATTERN = /^pages-.*\.stories\.tsx$/;
...
/**
 * Extract all story exports from the split pages-*.stories.tsx files
 * (formerly a single pages.stories.tsx, split across files that share
 * the same meta title so IDs are unaffected).
 *
...
function extractStorybookStories() {
  const storyFiles = fs
    ...
    .filter((name) => STORIES_FILE_PATTERN.test(name));

  const stories = [];

  for (const fileName of storyFiles) {
    const content = fs.readFileSync(path.join(STORIES_DIR, fileName), "utf-8");
...
function generateSuggestedMappings(routes, stories, existingMap) {
  ...
  for (const { route, isDynamic } of routes) {
    ...
    for (const story of stories) {
      ...
      if (
        storyName.includes(normalizedRouteName) ||
        normalizedRouteName.includes(storyName)
      ) {
    ...
    if (matchedStory) {
      ...
      suggestions[route] =
        `?path=/story/📄-pages-application-pages--${normalizedRouteName}`;
    }
  }
...
function validateRoutes() {
  console.log(
    `${colors.bold}${colors.cyan}Storybook Route Validation${colors.reset}\n`,
  );
  ...
  console.log(
    `${colors.green}✓ Covered routes: ${covered.length}${colors.reset}`,
  );
  console.log(
    `${colors.red}✗ Missing routes: ${missing.length}${colors.reset}\n`,
  );

  if (missing.length > 0) {
    console.log(
      `${colors.bold}${colors.yellow}Missing Route Mappings:${colors.reset}`,
    );
    ...
    for (const { route, filePath } of missing) {
      ...
      if (suggestions[route]) {
        console.log(
          `  ${colors.cyan}Suggested:${colors.reset} '${route}': '${suggestions[route]}'`,
        );
      }
    }
    ...
    if (process.argv.includes("--update")) {
      ...
      console.log(
        `${colors.green}✓ Route map updated successfully${colors.reset}`,
      );
    } else {
      console.log(
    ...
    console.log(
      `${colors.green}${colors.bold}✓ All routes are mapped!${colors.reset}\n`,
    );
