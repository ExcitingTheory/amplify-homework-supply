...
The script can be opened in Recording Studio 3 for TTS generation or human recording.`,
    inputSchema: z.object({
      preset: z.enum(['word', 'conversation', 'question']).describe('The RS3 preset type'),
      title: z.string().describe('Title for the recording script'),
      scene: z.string().optional().describe('Scene description (e.g., "INT. CLASSROOM - MORNING")'),
      speakers: z.record(z.string(), z.object({
        name: z.string().describe('Display name of the speaker'),
        voice: z.enum(['alloy', 'echo', 'fable', 'onyx', 'nova', 'shimmer']).optional().describe('TTS voice to use (default: alloy)'),
        description: z.string().optional().describe('Character description (e.g., "30s, energetic, teacher")'),
      })).describe('Map of speaker IDs to speaker config. For "word" preset use keys "phrase_track" and "definition_track". For "question" preset use "prompt_track" and "answer_track". For "conversation" use any descriptive keys.'),
      dialogue: z.array(z.object({
        speaker: z.string().describe('Speaker ID matching a key in the speakers object'),
        text: z.string().describe('The line of dialogue or text to speak'),
        direction: z.string().optional().describe('Acting/delivery direction (e.g., "slowly, with emphasis")'),
        emotion: z.string().optional().describe('Emotional tone (e.g., "cheerful", "serious", "questioning")'),
      })).describe('Ordered array of dialogue lines'),
      // For word preset - optional fields extracted from dialogue
      pronunciation: z.string().optional().describe('Phonetic pronunciation for word preset (e.g., "koh-hee")'),
      // For updating existing records
      existingWordId: z.string().optional().describe('ID of an existing Word to update with this script (word preset only)'),
      existingFileId: z.string().optional().describe('ID of an existing File to update with this script (conversation/question presets only)'),
    }),
    execute: async ({ preset, title, scene, speakers, dialogue, pronunciation, existingWordId, existingFileId }: any) => {
      ...
      if (preset === 'word') {
        ...
        if (!speakerIds.includes('definition_track')) {
          warnings.push('Word preset should include "definition_track" speaker');
        }
      } else if (preset === 'question') {
        if (!speakerIds.includes('prompt_track')) {
          warnings.push('Question preset should include "prompt_track" speaker');
        }
        if (!speakerIds.includes('answer_track')) {
          warnings.push('Question preset should include "answer_track" speaker');
        }
      }
      ...
      if (unknownSpeakers.length > 0) {
        warnings.push(`Dialogue references undefined speakers: ${[...new Set(unknownSpeakers)].join(', ')}`);
      }
      ...
      const lockedTracks = preset === 'word'
      ...
      if (preset === 'word') {
        const phraseLines = dialogue.filter((d: any) => d.speaker === 'phrase_track');
        const definitionLines = dialogue.filter((d: any) => d.speaker === 'definition_track');
        wordData = {
      ...
      if (preset === 'conversation' || preset === 'question') {
        fileData = {
          name: `${title.replace(/[^a-zA-Z0-9-_ ]/g, '').replace(/\s+/g, '-').toLowerCase()}.json`,
      ...
      return {
        ...
        preview: {
          ...
          speakers: Object.entries(speakers).map(([id, config]: [string, any]) => ({
            id,
            name: config.name,
            voice: config.voice || 'alloy',
          })),
          ...
          dialoguePreview: dialogue.slice(0, 5).map((line: any) => ({
            ...
            text: line.text.length > 80 ? line.text.substring(0, 80) + '...' : line.text,
          })),
          ...(wordData ? { wordPhrase: wordData.phrase, wordDefinition: wordData.definition } : {}),
