 ...
 *
 * updateGuildXP now uses getOrCreateStudentProfile to get cohortId, then queries
 * guilds via listGuildByCohortId and checks guild.members array for membership.
 */
  ...
  it("should update guild totalXP and return guild IDs", async () => {
    ...
    const updateCall = mockGraphql.mock.calls.find(
      (call: any) =>
        ...
        call[0].query.includes("updateGuild"),
    );
