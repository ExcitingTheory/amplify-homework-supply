...
// Mock S3 — use vi.hoisted() so the object is available inside the hoisted vi.mock factory
const s3Holder = vi.hoisted(() => ({ send: vi.fn() }));
vi.mock("@aws-sdk/client-s3", () => {
  const holder = s3Holder;
  return {
...
// Convenience alias
const mockS3Send = s3Holder.send;
...
describe("gamification handler — generateSkillTree", () => {
      ...
      root: {
        children: [
          ...
          },
          {
            ...
            children: [
              {
                text: "The chloroplast is the organelle where photosynthesis occurs.",
              },
            ],
          },
        ],
    ...
    mockGraphql
            ...
            items: [
              {
                word: {
                  ...
                  word: "chloroplast",
                  phonetic: null,
                  definition: "organelle for photosynthesis",
                },
              },
              {
                word: {
      ...
      .mockResolvedValueOnce({
        data: {
          createSkill: {
            id: "skill-new-1",
            title: "Identify plant organelles",
      ...
      .mockResolvedValueOnce({
        data: {
          createSkill: {
            id: "skill-new-2",
            title: "Explain photosynthesis steps",
            ...
            content: JSON.stringify({
              skills: [
                ...
                },
                {
                  title: "Explain photosynthesis steps",
                  description: "Light and dark reactions",
                  prerequisites: ["Identify plant organelles"],
                  xpReward: 50,
                },
              ],
            }),
  ...
  it("handles unit with no content gracefully", async () => {
    ...
    mockGraphql
      ...
      .mockResolvedValueOnce({
        data: { listQuestionUnitByUnitID: { items: [] } },
      })
      .mockResolvedValueOnce({
      ...
      .mockResolvedValueOnce({
        data: {
          getUnit: {
            ...
            name: "Grammar",
            description: null,
            data: '{"root":{"children":[{"children":[{"text":"Nouns and verbs"}]}]}}',
          },
        },
      })
      .mockResolvedValueOnce({ data: { listUnitWordByUnitID: { items: [] } } })
      ...
      .mockResolvedValueOnce({
        data: {
          getUnit: {
            ...
            name: "Test",
            description: null,
            data: '{"root":{"children":[{"children":[{"text":"Some content"}]}]}}',
          },
        },
      })
      .mockResolvedValueOnce({ data: { listUnitWordByUnitID: { items: [] } } })
...
describe("extractTextFromLexicalJSON (internal)", () => {
  ...
  it("handles nested lexical nodes", async () => {
    ...
    mockS3Send.mockResolvedValueOnce({
      Body: {
        transformToString: () =>
          Promise.resolve(JSON.stringify(complexLexical)),
      },
    });
