 ...
 */
export async function extractIMSCC(s3Key: string): Promise<FormatExtractionResult> {
  ...
  for (const resource of resources) {
    ...
    if (type.includes('webcontent') && href) {
      ...
      if (contentFile && (href.endsWith('.html') || href.endsWith('.htm') || href.endsWith('.xhtml'))) {
  ...
  console.log(`[extractIMSCC] Extracted ${text.length} chars, ${directQuestions.length} questions, ${directObjectives.length} objectives`);
  ...
  return {
    ...
    directContent: {
      ...
      objectivesJSON: directObjectives.length > 0 ? directObjectives : undefined,
    },
  };
}
 ...
 */
export async function extractSCORM(s3Key: string, buffer: Buffer, zip: any, manifest: any): Promise<FormatExtractionResult> {
  ...
  const schemaVersion = findValue(manifest, 'schemaversion');
  const sourceFormat = schemaVersion && schemaVersion.includes('2004') ? 'scorm-2004' : 'scorm-1.2';
  ...
  for (const resource of resources) {
    ...
    if (file && (href.endsWith('.html') || href.endsWith('.htm') || href.endsWith('.xhtml'))) {
  ...
  console.log(`[extractSCORM] Extracted ${text.length} chars, ${directObjectives.length} objectives`);
 ...
 */
export async function extractQTI(s3Key: string): Promise<FormatExtractionResult> {
  ...
  const sourceFormat = xmlContent.includes('qti/3') || xmlContent.includes('qtiv3') ? 'qti-3.0' : 'qti-2.1';
  
  console.log(`[extractQTI] Extracted ${questions.length} questions (${sourceFormat})`);
...
function parseQTIXml(parser: any, xmlContent: string): unknown[] {
  ...
  for (const item of assessmentItems) {
    ...
    const feedback = findFirst(item, 'feedbackInline') || findFirst(item, 'feedbackBlock');
 ...
 */
function parseInteraction(item: any): { questionType: string; options: string[]; correctAnswer: string } {
      ...
      if (interactionName === 'choiceInteraction') {
        ...
        for (const choice of choices) {
          ...
          if (correctValues.includes(choiceId)) {
            correctAnswer = correctAnswer ? `${correctAnswer}; ${choiceText}` : choiceText;
          }
        }
      }
