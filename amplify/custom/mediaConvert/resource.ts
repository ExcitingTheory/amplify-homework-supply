...
export class MediaConvertConstruct extends Construct {
  ...
  constructor(scope: Construct, id: string, props: MediaConvertConstructProps) {
    ...
    this.mediaConvertRole = new iam.Role(this, 'MediaConvertServiceRole', {
      ...
      description: 'Allows AWS MediaConvert to read source files and write HLS output to S3',
    });

    bucket.grantRead(this.mediaConvertRole);
    bucket.grantWrite(this.mediaConvertRole);
    ...
    const jobStateRule = new events.Rule(this, 'MediaConvertJobStateRule', {
      description: 'Routes MediaConvert COMPLETE/ERROR events to the handler Lambda',
      eventPattern: {
    ...
    // S3 read access (required when invoked by S3 event to read object metadata)
    bucket.grantRead(handlerLambda);
  }
}
