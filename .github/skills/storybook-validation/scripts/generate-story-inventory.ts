 ...
 * 
 * @module storybook-validation/scripts/generate-story-inventory
 */
          ...
          if (ts.isPropertyAssignment(prop) && ts.isIdentifier(prop.name)) {
            ...
            if (propName === 'title' && ts.isStringLiteral(prop.initializer)) {
              ...
              if (titleParts.length > 1) {
                category = titleParts[0].replace(/[^\w\s]/g, '').trim() || 'Components';
              }
            } else if (propName === 'component' && ts.isIdentifier(prop.initializer)) {
      ...
      if (isExported) {
        node.declarationList.declarations.forEach((decl) => {
          if (ts.isIdentifier(decl.name) && decl.name.text !== 'default') {
            ...
            const lineNumber = sourceFile.getLineAndCharacterOfPosition(decl.getStart()).line + 1;
            variants.push({
              name: variantName,
              hasPlayFunction,
              hasParameters,
...
`;
  ...
  sortedCategories.forEach(([category, categoryStories]) => {
    const variantCount = categoryStories.reduce((sum, s) => sum + s.totalVariants, 0);
    const withTests = categoryStories.filter(s => s.hasInteractionTests).length;
...
*Generated by .github/skills/storybook-validation/scripts/generate-story-inventory.ts*
`;
...
async function main() {
  ...
  console.log(`📊 Total: ${stories.length} stories, ${stories.reduce((sum, s) => sum + s.totalVariants, 0)} variants`);
}
