 ...
 *
 * Usage:
 *   npx tsx .github/skills/storybook-audit/scripts/validate-component-mocks.ts [componentName]
 *
 * @module storybook-audit/scripts/validate-component-mocks
 */
...
function validateMockData(
  ...
  const componentName = path
    .basename(storyInfo.storyFile, path.extname(storyInfo.storyFile))
    .replace(".stories", "");
    ...
    mockData.messages.forEach((msg: any, index: number) => {
      if (!msg.parts) {
        issues.push({
          ...
          suggestion:
            'Update to current format: { id, role, parts: [{ type: "text", text: "..." }] }',
        });
      }
