...
Read the full backend logs — do NOT truncate with `tail` or `head`. Issues at the top of logs are just as important as recent ones.

```bash
cat test/logs/sandbox.log
```

Look for: errors, exceptions, DynamoDB throttling, auth failures, subscription/websocket issues.

### Phase 5: Verify Next.js Dev Server Logs (2-3 minutes)

Read the full Next.js dev server log:

```bash
cat test/logs/next-dev.log
```

Look for: server-side 500 errors, SyntaxErrors, TypeErrors, module resolution failures, JSON parsing errors.

### Phase 6: Verify Collaboration Logs (1-2 minutes)

Read the full collaboration server log if it exists:

```bash
if [ -f test/logs/collab.log ]; then
  cat test/logs/collab.log
else
  echo "No test/logs/collab.log found — collab server may not be running"
fi
```

Look for: connection errors, sync failures, document-not-found, awareness issues, disconnects/timeouts.
