...
for namespace in "${NAMESPACES[@]}"; do
  for lang in "${TARGET_LANGS[@]}"; do
    ...
    if [ $CURRENT_JOB -lt $TOTAL_JOBS ]; then
      echo "  Waiting 3 seconds before next translation..."
      sleep 3
    fi
  done
done
...
echo "  2. Verify translations: npx tsx scripts/verify-translations.ts"
echo "  3. Run reverse translation: npx tsx scripts/reverse-translate.ts <namespace> en <lang>"
echo "  4. Generate report: npx tsx scripts/generate-translation-report.ts"
echo ""
