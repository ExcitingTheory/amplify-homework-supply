...
if (fs.existsSync(muiPropsPath)) {
  ...
  console.log(`📦 Loaded ${muiProps.length} Material UI prop names for i18n exclusion`);
} else {
  console.warn('⚠️  MUI props not found. Run `npm run extract:mui-props` to generate.');
}
...
const commonCssValues = [
  ...
  'start', 'end', 'space-between', 'space-around', 'space-evenly', 'stretch',
  ...
  'vertical', 'horizontal',
  'pointer', 'default', 'move', 'text', 'wait', 'help', 'not-allowed',
...
const i18nextRuleConfig = [
  'warn',
  {
    ...
    'jsx-components': {
      exclude: ['Icon', 'IconButton', 'SvgIcon', 'MenuItem', 'Option', 'svg', 'path', 'circle', 'rect', 'line', 'polyline', 'polygon', 'ellipse', 'g', 'defs', 'clipPath', 'mask', 'pattern', 'linearGradient', 'radialGradient', 'stop', 'use', 'symbol', 'text', 'tspan'],
    },
    ignore: [
      '^\\s*$', // Empty strings
      '^[0-9]+$', // Numbers only
      ...
      'ex$', 'cm$', 'mm$', 'in$', 'pt$', 'pc$', 'deg$', 'rad$', 'grad$', 
      'turn$', 's$', 'ms$', 'fr$',
