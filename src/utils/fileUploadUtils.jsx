...
import {
  ...
  analyzeDocument as analyzeDocumentAction,
...
export async function generateEmbeddings(fileID) {
  try {
    const result = await generateFileEmbeddings(fileID);
    ...
    throw new Error(
      error.message || "Failed to generate embeddings - unknown error",
    );
  }
}
...
async function triggerEmbeddingGeneration(fileID) {
  try {
    ...
    if (!result.success) {
      ...
      console.log(
        "[Embedding] Started background processing for file:",
        fileID,
      );
    }
  } catch (error) {
...
async function triggerEmbeddingsGeneration(fileID) {
  try {
    ...
    if (!result.success) {
      ...
      console.log(
        "[Embeddings] Started background processing for file:",
        fileID,
      );
    }
  } catch (error) {
...
export async function uploadFile(
  ...
  if (isAnalyzableDocument(correctedMimeType)) {
    ...
    const { data: newDocument, errors: documentErrors } =
      await amplifyClient.models.Document.create(documentData);

    if ((documentErrors && documentErrors.length > 0) || !newDocument) {
      console.error("Error creating Document record:", documentErrors);
      throw new Error(
        documentErrors?.[0]?.message || "Failed to create Document record",
      );
    }
  ...
  const { data: newFile, errors: fileErrors } =
    await amplifyClient.models.File.create(fileData);
  ...
  const embedding = await triggerEmbeddingGeneration(fileModel.id).catch(
    (error) => {
  ...
  if (documentModel && unitId) {
    try {
      const amplifyClient = getAmplifyClient();
      const { data: unit, errors: unitErrors } =
        await amplifyClient.models.Unit.get({ id: unitId });
      ...
      if ((unitErrors && unitErrors.length > 0) || !unit) {
        ...
        const { data: unitDoc, errors: unitDocErrors } =
        ...
        if ((unitDocErrors && unitDocErrors.length > 0) || !unitDoc) {
          console.warn(
            "Error creating UnitDocument relationship:",
            unitDocErrors,
          );
        } else {
...
async function waitForDocumentSync(documentId) {
  return new Promise((resolve, reject) => {
    const timeout = setTimeout(() => {
      ...
      reject(
        new Error(
          `Document sync timeout after 10 seconds for document: ${documentId}`,
        ),
      );
    }, 10000);
    ...
    const unsubscribe = Hub.listen("datastore", ({ payload }) => {
      if (
        payload.event === "outboxMutationProcessed" &&
...
export async function analyzePDF(fileID) {
  try {
    const result = await analyzeDocumentAction(fileID);
...
export async function cancelPDFAnalysis(fileId) {
  const result = await cancelDocumentAnalysisAction(fileId);
...
export async function uploadAndAnalyzePDF(
  ...
  if (!isAnalyzableDocument(correctedType)) {
    throw new Error(
      `File type ${correctedType} is not analyzable. Supported types: PDF, Word, Excel, PowerPoint, text, markdown, CSV, ODF, EPUB, SCORM, IMS CC, QTI, GIFT`,
    );
  }

  // Upload the file
  const { fileModel, documentModel } = await uploadFile(
    file,
    identityId,
    unitId,
    onProgress,
