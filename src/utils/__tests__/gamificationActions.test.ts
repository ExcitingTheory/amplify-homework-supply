 ...
 *
 * NOTE: src/utils/gamificationActions.ts has been removed and replaced by
 * app/actions/gamification.ts (server actions). These tests are skipped
 * until a new test file is created for the server action variants.
 * See: app/actions/gamification.ts
 */
...
// The module was moved — mock to prevent resolution errors
...
describe.skip("gamificationActions (legacy — module moved to app/actions/gamification)", () => {
  ...
  describe("awardXPAndCheck", () => {
    it("returns parsed XP result and fires badge/streak/guild calls", async () => {
      ...
      expect(mockMutations.checkBadges).toHaveBeenCalledWith({
        studentId: "student-1",
      });
    ...
    it("handles badge/streak failures gracefully (fire-and-forget)", async () => {
      ...
      mockMutations.checkBadges = vi
        .fn()
        .mockRejectedValue(new Error("badge fail"));
  ...
  describe("checkPersonalBest", () => {
    ...
    it("returns null on exception", async () => {
      mockMutations.checkPersonalBest = vi
        .fn()
  ...
  describe("checkEasterEggs", () => {
    it("returns discovered eggs", async () => {
      mockMutations.checkEasterEggs = vi.fn().mockResolvedValue({
        data: {
          discovered: [{ eggId: "egg-1", message: "Found it!", xpReward: 25 }],
        },
        errors: null,
      });
  ...
  describe("discoverEasterEgg", () => {
    ...
    it("returns null on exception", async () => {
      mockMutations.discoverEasterEgg = vi
        .fn()
  ...
  describe("updateUnitMemoryAndRebuild", () => {
    it("calls updateStudentUnitMemoryFromGrade then rebuilds profile", async () => {
      mockMutations.updateStudentUnitMemoryFromGrade = vi
        .fn()
        .mockResolvedValue({
          data: {},
          errors: null,
      ...
      expect(
        mockMutations.updateStudentUnitMemoryFromGrade,
      ...
      expect(mockMutations.rebuildStudentMemoryProfile).toHaveBeenCalledWith({
        studentId: "student-1",
      });
    });
  ...
  describe("generateSkillTree", () => {
    ...
    it("returns null on exception", async () => {
      mockMutations.generateSkillTree = vi
        .fn()
