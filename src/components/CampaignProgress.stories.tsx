...
export const InProgress: Story = {
  args: {
    ...
    setting:
      "In a world where bugs roam free and code entropy threatens all, a brave team of developers must band together to defeat the Algorithm Dragon before it corrupts the entire codebase.",
  ...
  play: async ({ canvasElement }) => {
    ...
    await canvas.findByText("The Algorithm Dragon");
  },
};
...
export const Completed: Story = {
  args: {
    ...
    setting:
      "The sprint deadline approaches and only maximum effort will save the release.",
    currentXP: 1200,
    targetXP: 1000,
    active: false,
    chapters: [
  ...
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    await canvas.findByText("Sprint to Victory");
  },
};
...
export const UrgentDeadline: Story = {
  ...
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    await canvas.findByText("The Midnight Challenge");
  },
};
...
export const SimpleProgress: Story = {
  ...
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    await canvas.findByText("Weekly XP Goal");
  },
};
...
export const EndedCampaign: Story = {
  ...
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    await canvas.findByText("The Legacy Refactor");
  },
};
