...
export default function JoinPeerReviewDialog({
  ...
  const hasPendingInvitations = useMemo(
    () =>
      notifications.some(
        (n) => n.type === "PEER_REVIEW_INVITE" && !n.interacted,
      ),
    [notifications],
  );

  // Default to the Invitations tab whenever the dialog opens with pending invites
  useEffect(() => {
    if (open) setTab(hasPendingInvitations ? 1 : 0);
  }, [open, hasPendingInvitations]);
          ...
          <Tab
            label={
              hasPendingInvitations
                ? `${t("peerReview.joinDialog.tabInvitations")} \u2022`
                : t("peerReview.joinDialog.tabInvitations")
            }
          />
        </Tabs>
