...
export const ToolCallPreview: React.FC<ToolCallPreviewProps> = ({
  ...
  const getStateLabel = (currentState: string) => {
    switch (currentState) {
      case "pending":
        return t("chatSidebar.toolCallPreview.stateLabels.pending");
      case "confirmed":
        return t("chatSidebar.toolCallPreview.stateLabels.confirmed");
      case "executing":
        return t("chatSidebar.toolCallPreview.stateLabels.executing");
      case "executed":
        return t("chatSidebar.toolCallPreview.stateLabels.executed");
      case "error":
        return t("chatSidebar.toolCallPreview.stateLabels.error");
      default:
        return currentState;
    }
  };
    ...
    if (isEditing && state === "pending") {
      ...
      if (def?.enum) {
        return (
          ...
          >
            <option value="">
              {t("chatSidebar.toolCallPreview.selectPlaceholder")}
            </option>
      ...
      if (paramType === "boolean") {
        return (
          <TextField
            ...
            onChange={(e) =>
              handleParameterChange(paramName, e.target.value === "true")
            }
            SelectProps={{ native: true }}
          >
            <option value="true">
              {t("chatSidebar.toolCallPreview.true")}
            </option>
            <option value="false">
              {t("chatSidebar.toolCallPreview.false")}
            </option>
          </TextField>
        );
      }
