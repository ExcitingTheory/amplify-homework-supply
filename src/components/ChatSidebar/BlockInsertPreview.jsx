  ...
  return (
    <Box>
      ...
      <Box sx={{ display: "flex", gap: 1, mb: 2 }}>
        <Chip
          label={`${questionGroups.length} questions`}
          size="small"
          color="primary"
          variant="outlined"
        />
      ...
      {questionGroups.map((group, idx) => (
        <Box key={idx} sx={{ mb: 3 }}>
          ...
          <QuizComponent nodeKey={`preview-quiz-${idx}`} data={group.answers} />
        </Box>
      ))}
    </Box>
  );
  ...
  <Box>
    ...
    <Box sx={{ display: "flex", gap: 1, mb: 2, flexWrap: "wrap" }}>
      ...
      {preview?.inputMethods && (
        <Chip
          label={`Input: ${preview.inputMethods.join(", ")}`}
          size="small"
          variant="outlined"
        />
      )}
  ...
  return (
    <Box>
      ...
      <Box sx={{ display: "flex", gap: 1, mb: 2 }}>
        ...
        {preview?.modes && (
          <Chip
            label={`Modes: ${preview.modes.join(", ")}`}
            size="small"
            color="info"
            variant="outlined"
          />
  ...
  <Box>
    ...
    <Box sx={{ display: "flex", gap: 1, flexWrap: "wrap", mb: 2 }}>
      ...
      {preview?.promptMethods && (
        <Chip
          label={`Prompt: ${preview.promptMethods.join(", ")}`}
          size="small"
          color="info"
          variant="outlined"
        />
    ...
    <Box>
      <Box sx={{ display: "flex", alignItems: "center", gap: 1, mb: 2 }}>
        ...
        <Typography variant="h6">
          {preview?.title || "Content Block"}
        </Typography>
      </Box>
      <Box sx={{ display: "flex", gap: 1, mb: 2, flexWrap: "wrap" }}>
        {preview?.contentType && (
          <Chip
            label={preview.contentType}
            size="small"
            color="primary"
            variant="outlined"
          />
        ...
        {preview?.nodeCount && (
          <Chip
            label={`${preview.nodeCount} elements`}
            size="small"
            variant="outlined"
          />
        )}
...
const BlockInsertPreview = ({ toolOutput, onInsertBlock, onReject }) => {
  ...
  return (
    ...
    >
      ...
      {blockType === "quiz" && (
        <QuizBlockPreview preview={preview} blockData={blockData} />
      )}
      {blockType === "answer" && (
        <AnswerBlockPreview preview={preview} blockData={blockData} />
      )}
      {blockType === "meaning-association" && (
        <MeaningAssociationPreview preview={preview} blockData={blockData} />
      )}
      {blockType === "custom-answer" && (
        <CustomAnswerPreview preview={preview} blockData={blockData} />
      )}
      {blockType === "content" && (
        <ContentBlockPreview preview={preview} blockData={blockData} />
      )}
