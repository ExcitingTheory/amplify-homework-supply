...
function TruncatedText({
  ...
  const content = markdown
    ? renderSimpleMarkdown(displayText)
    : searchTerm
      ? highlightMatches(displayText, searchTerm)
      : displayText;
...
export default function SearchResults({
  ...
  const handleOpenAssignment = (assignment) => {
    if (assignment.sectionId) {
      handleOpenSection({ id: assignment.sectionId });
    } else if (assignment.unitId) {
      handleOpenUnit({ id: assignment.unitId });
    }
  };
    ...
    try {
      ...
      if (itemType === "word") {
        ...
        const { data: existing } = await client.models.UnitWord.list({
          filter: {
            and: [{ unitID: { eq: unitId } }, { wordID: { eq: item.id } }],
          },
        });
        ...
        const { data: existing } = await client.models.QuestionUnit.list({
          filter: {
            and: [{ unitID: { eq: unitId } }, { questionID: { eq: item.id } }],
          },
        });
      ...
      console.log(
        `[SearchResults] Linked ${itemType} ${item.id} to unit ${unitId}`,
      );
    } catch (error) {
        ...
        >
          ...
          >
            ...
            <Box sx={{ flex: 1, minWidth: 0 }}>
              ...
              >
                {searchQuery
                  ? highlightMatches(file.name, searchQuery)
                  : file.name}
              </Typography>
              {file.page && (
                <Chip
          ...
          >
            ...
            <Tooltip title={t("chatSidebar.searchResults.openFile")}>
              <IconButton size="small" onClick={() => handleOpenFile(file)}>
        ...
        >
          ...
          >
            ...
            <Box sx={{ flex: 1, minWidth: 0 }}>
              ...
              >
                {searchQuery
                  ? highlightMatches(word.phrase, searchQuery)
                  : word.phrase}
              </Typography>
              {word.phonetic && (
                <Typography
                ...
                >
                  {searchQuery
                    ? highlightMatches(word.phonetic, searchQuery)
                    : word.phonetic}
                </Typography>
              )}
            </Box>
          ...
          >
            ...
            <ButtonGroup size="small" variant="outlined">
              {onInsertWord && (
                <Tooltip
                  title={t("chatSidebar.searchResults.insertIntoEditor")}
                >
                  <IconButton
                    size="small"
              ...
              {unitId && (
                <Tooltip
                  title={
                    linkState === "linked"
                      ? t("chatSidebar.searchResults.linkedToUnit")
                      : t("chatSidebar.searchResults.addToUnit")
                  }
                >
                  <span>
                    <IconButton
                      ...
                      disabled={
                        linkState === "loading" || linkState === "linked"
                      }
              ...
              <Tooltip title={t("chatSidebar.searchResults.editWord")}>
                <IconButton size="small" onClick={() => handleOpenWord(word)}>
    ...
    return (
      ...
      >
        ...
        >
          ...
          >
            <QuestionIcon
              sx={{ color: "info.main", fontSize: 28, flexShrink: 0, mt: 0.25 }}
            />
            <Typography
              variant="body1"
              sx={{
            ...
            >
              {searchQuery
                ? highlightMatches(question.prompt, searchQuery)
                : question.prompt}
            </Typography>
          </Box>
          <Box
        ...
        >
          ...
          >
            ...
            <Box sx={{ flex: 1, minWidth: 0 }}>
              ...
              >
                {searchQuery
                  ? highlightMatches(unit.name, searchQuery)
                  : unit.name}
              </Typography>
              {unit.published && (
                <Chip
          ...
          >
            ...
            <Tooltip title={t("chatSidebar.searchResults.openUnit")}>
              <IconButton size="small" onClick={() => handleOpenUnit(unit)}>
                <OpenIcon fontSize="small" />
              </IconButton>
            </Tooltip>
          </Box>
  ...
  const renderAssignmentResult = (assignment) => (
    <Box
      key={assignment.id}
      sx={{
        display: "flex",
      ...
      >
        ...
        >
          ...
          <Box sx={{ minWidth: 0 }}>
            ...
            >
              {assignment.name || assignment.title || "Assignment"}
            </Typography>
            {assignment.unitName && (
        ...
        <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
          <RelevanceScore similarity={assignment.similarity} />
          {(assignment.sectionId || assignment.unitId) && (
      ...
      {assignmentResults.length > 0 && (
        <Box>
          <SectionHeader
            ...
            color="warning"
            expanded={expandedSections.assignments !== false}
            onToggle={() => toggleSection("assignments")}
          />
          <Collapse in={expandedSections.assignments !== false} timeout={300}>
            <Box
              sx={{ display: "flex", flexDirection: "column", gap: 1.5, mt: 1 }}
            >
              {assignmentResults.map(renderAssignmentResult)}
            </Box>
          </Collapse>
        </Box>
      )}
