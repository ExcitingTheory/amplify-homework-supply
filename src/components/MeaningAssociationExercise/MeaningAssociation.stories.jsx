...
export default {
  title: "🧩 UI Components/Meaning Association Exercise",
  parameters: {
    layout: "padded",
  },
  decorators: [
  ...
  play: async ({ canvasElement }) => {
    ...
    await waitFor(
      () => {
        ...
        expect(
          canvas.getByText(
            /A greeting|A farewell|Expression of gratitude|Polite request/i,
          ),
        ).toBeInTheDocument();
      },
      { timeout: 5000 },
    ...
    const definitions = canvas.getAllByText(
      /A greeting|A farewell|Expression of gratitude|Polite request/i,
    );
...
export const CompletionScreenEasy = {
  ...
  play: async ({ canvasElement }) => {
    ...
    expect(
      canvas.getByRole("button", { name: /Hard Mode/i }),
    ).toBeInTheDocument();
  },
};
