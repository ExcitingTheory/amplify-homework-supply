...
const ChatSidebar = ({ onClose }) => {
  ...
  const deepCloneMessages = useCallback((messages) => {
    ...
    try {
      ...
      console.log(
        "[ChatSidebar] deepCloneMessages: cloned",
        withIds.length,
        "messages",
      );
      return withIds;
      ...
      case ACTIONS.REMOVE_UPLOADED_FILE:
        return {
          ...
          documentProcessingStatus: Object.fromEntries(
            Object.entries(state.documentProcessingStatus).filter(
              ([index]) => parseInt(index) !== action.payload,
            ),
          ),
        };
  ...
  const { documents: filesContextDocuments } =
  ...
  const studentMemoryVersionRef = React.useRef(0);
  React.useEffect(() => {
    ...
    const subscription = client.models.StudentMemory.observeQuery({
      ...
      error: (err) =>
        console.warn(
          "[ChatSidebar] StudentMemory subscription error:",
          err?.message,
        ),
    });
  ...
  useEffect(() => {
    ...
    if (chatChanged) {
      ...
      console.log(
        "[ChatSidebar] Loading messages for chat:",
        assistantChat.id,
        "count:",
      ...
      client.models.AssistantChatFile.list({
        ...
        .catch((err) =>
          console.error("[ChatSidebar] Error loading files:", err),
        );
    }
  ...
  React.useEffect(() => {
    if (vectorStoreCtx?.search) {
      ...
      console.log(
        "[ChatSidebar] Registered vector store search, isReady:",
        vectorStoreCtx.isReady,
      );
    }
        ...
        tools[toolName] = {
          ...
          execute: async (args) => {
            ...
            try {
              ...
              console.error(
                `[ChatSidebar] Error executing ${toolName}:`,
                error,
              );
              return {
                success: false,
  ...
  const flushSave = useCallback(async () => {
    ...
    try {
      ...
      const { data: fresh } = await client.models.AssistantChat.get({
        id: pending.chatId,
      });
      if (!fresh) {
        console.warn(
          "[ChatSidebar] Chat not found for save, skipping:",
          pending.chatId,
        );
        return;
      }
  ...
  const saveDraft = useCallback(
    (draftText, chat, currentMessages = []) => {
      if (!chat?.id) return;
  ...
  const saveFileAssociations = useCallback(async (files, chat) => {
    ...
    try {
      ...
      const { data: currentAssociations } =
        await client.models.AssistantChatFile.list({
      ...
      const associationsToRemove = currentAssociations.filter(
        (a) => !uploadedFileIds.has(a.fileID),
      );
  ...
  const {
    processResponse: processNailedIt,
    showCelebration,
    dismissCelebration,
    lastResult: nailedItResult,
  ...
  const chatHookResult = useChat({
    ...
    experimental_sendAutomaticallyWhen:
      lastAssistantMessageIsCompleteWithToolCalls,
  ...
  useEffect(() => {
    if (
      ...
      Object.keys(filesContextDocuments).length === 0
    )
      return;
    ...
    Object.entries(updated).forEach(([index, status]) => {
      if (status.documentId && documentStatuses[status.documentId]) {
        ...
        if (docStatus === "completed" && status.status !== "analyzed") {
          ...
          docStatus === "uploaded" &&
          ["analyzing", "extracting"].includes(status.status)
        ) {
  ...
  const cancelProcessing = async (index) => {
    ...
    try {
      console.log(
        "[ChatSidebar] Cancelling analysis for document:",
        status.documentId,
      );
          ...
          >
            <Box sx={{ textAlign: "center", color: "white" }}>
              ...
              <Typography variant="h6">
                {t("chatSidebar.dropFilesHere")}
              </Typography>
              <Typography variant="body2">
                {t("chatSidebar.attachFilesMessage")}
              </Typography>
            </Box>
          </Box>
        )}
                  ...
                  onClick={async () => {
                    try {
                      ...
                      if (
                        ...
                        console.log(
                          "[ChatSidebar] Saving current chat state before creating new chat",
                        );
                        await saveDraft(input, assistantChat, messages);
                      }
                      ...
                      if (result.errors && result.errors.length > 0) {
                        console.error(
                          "[ChatSidebar] Errors creating chat:",
                          result.errors,
                        );
                      ...
                      if (!result.data) {
                        console.error(
                          "[ChatSidebar] No data returned from create. Full result:",
                          JSON.stringify(result, null, 2),
                        );
                        throw new Error(
                          "Failed to create chat - no data returned. Check that backend is deployed and accessible.",
                        );
                      }

                      console.log(
                        "[ChatSidebar] Created new chat:",
                        result.data.id,
                        "at:",
                      ...
                      console.error(
                        "[ChatSidebar] Error creating new chat:",
                        error,
                      );
                      alert(
                        "Failed to create new chat. Please check that the backend is running and try again.",
                      );
                    }
                  }}
                  ...
                  onClick={async () => {
                    ...
                    if (assistantChat?.id && !isLoadingChat) {
                      try {
                        ...
                        await flushSave();
                        const { data: freshChat } =
                          await client.models.AssistantChat.get({
                            id: assistantChat.id,
                          });
                        await client.models.AssistantChat.update({
                          id: assistantChat.id,
                        ...
                        console.log(
                          "[ChatSidebar] Chat archived - ChatContext will create new chat",
                        );
                        ...
                        console.error(
                          "[ChatSidebar] Error archiving chat:",
                          error,
                        );
                      }
                    }
                    ...
                    <>
                      ...
                      <List sx={{ p: 0 }}>
                        {chatHistories
                          ...
                          .map((history, index) => {
                            console.log(
                              "[ChatSidebar] Rendering chat history:",
                              history.id,
                              history,
                            );
                            ...
                            try {
                              ...
                              console.error(
                                "[ChatSidebar] Error parsing messages:",
                                err,
                              );
                              historyMessages = [];
                            }
                            ...
                            console.log(
                              "First user message for history",
                              history.id,
                              ":",
                              firstUserMessage,
                              historyMessages,
                            ...
                            let preview =
                              historyDraft || t("chatSidebar.emptyChat");
                            if (
                                  ...
                                  onClick={async () => {
                                    if (assistantChat && input.trim()) {
                                      try {
                                        ...
                                        console.error(
                                          "[ChatSidebar] Error saving draft:",
                                          error,
                                        );
                                      }
                                    }
                                    ...
                                    console.log(
                                      "[ChatSidebar] Switching to chat history:",
                                      history.id,
                                    );
                                  }}
                                  selected={isActive}
                                  ...
                                  <ListItemText
                                    ...
                                    secondary={
                                      <Typography
                                        ...
                                        color="text.secondary"
                                      >
                                        {messageCount} messages • {createdAt}
                                        {isArchived ? " • Archived" : ""}
                                      </Typography>
                                    }
                                  />
                              ...
                              <React.Fragment key={history.id}>
                                <ListItemButton
                                  onClick={async () => {
                                    ...
                                    console.log(
                                      "[ChatSidebar] Switching to archived chat:",
                                      history.id,
                                    );
                                  }}
                                  selected={isActive}
                                              ...
                                              await client.models.AssistantChat.update(
                                                {
                                                  ...
                                                  archived: false,
                                                  _version:
                                                    freshHistory?._version,
                                                },
                                              );
                                              console.log(
                                                "[ChatSidebar] Unarchived chat:",
                                                history.id,
                                              );
                                            } catch (error) {
                                              console.error(
                                                "[ChatSidebar] Error unarchiving chat:",
                                                error,
                                              );
                                            }
                                          } else {
                                            console.warn(
                                              "[ChatSidebar] Cannot unarchive legacy chat",
                                            );
                                          }
                                        }}
                                        disabled={isLegacy}
                                ...
                                {index <
                                  chatHistories.filter((h) => h.archived)
                                    .length -
                                    1 && <Divider sx={{ my: 0.5 }} />}
                              </React.Fragment>
                            );
                      ...
                      if (toolName === "search_content") {
                        ...
                        if (part.state === "output-available" && part.output) {
                          try {
                            ...
                            console.error(
                              "[ChatSidebar] Failed to parse search output:",
                              e,
                            );
                          }
                        }
                                ...
                                <Box sx={{ width: "100%" }}>
                                  {parsedOutput.success ? (
                                    <>
                                      ...
                                      >
                                        {t("chatSidebar.foundResults", {
                                          count:
                                            parsedOutput.results?.length || 0,
                                        })}
                                      </Typography>
                                      <SearchResults
                      ...
                      if (
                        ...
                        if (part.state === "output-available" && part.output) {
                          try {
                            ...
                            console.error(
                              "[ChatSidebar] Failed to parse block output:",
                              e,
                            );
                          }
                        }
                      ...
                      if (toolName === "create_recording_script") {
                        ...
                        if (part.state === "output-available" && part.output) {
                          try {
                            ...
                            console.error(
                              "[ChatSidebar] Failed to parse recording script output:",
                              e,
                            );
                          }
                        }
                        ...
                        return (
                          ...
                          >
                            ...
                            {part.state === "input-available" && (
                              ...
                              >
                                Creating {part.input?.preset} recording
                                script...
                              </Typography>
                            )}
                        ...
                        return (
                          ...
                          >
                            ...
                            {part.state === "input-available" && (
                              ...
                              >
                                {t("chatSidebar.contentTypeAndTopic", {
                                  contentType: part.input?.contentType,
                        ...
                        return (
                          ...
                          >
                            ...
                            {part.state === "input-available" && (
                              ...
                              >
                                Topic: {part.input?.topic} ({part.input?.count}{" "}
                                {part.input?.questionType} questions,{" "}
                                {part.input?.difficulty} difficulty)
                              </Typography>
                            )}
                            ...
                            {part.state === "output-available" && (
                              ...
                              >
                                ✓{" "}
                                {part.output?.message ||
                                  `Generated ${part.output?.questionsGenerated || part.input?.count} questions`}
                              </Typography>
                            )}
        ...
        >
                    ...
                    {isDocument &&
                      ...
                      status.documentId && (
                        <IconButton
                          size="small"
                          onClick={() =>
                            openVocabularyReview(status.documentId)
                          }
                          sx={{ p: 0.5 }}
          ...
          {chatCreationError && (
            ...
            >
              ...
              >
                Possible causes: Backend not deployed, authentication issue, or
                network error
              </Typography>
              <Button
                size="small"
