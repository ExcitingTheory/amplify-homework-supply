...
import type {
  ...
  GroupStats,
  ParticipantProgress,
} from "../../yjs/PracticeCollaborationProvider";
...
const mockGroupStats: GroupStats = {
  totalParticipants: 4,
  averageAccuracy: 65,
  blocksCompletedTotal: 12,
  blockAccuracies: {},
  lastUpdated: new Date().toISOString(),
};
const mockOwnProgress: ParticipantProgress = {
  username: "alice",
  blocksCompleted: 3,
  blocksAttempted: 5,
...
export const Connected: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await canvas.findByText(/ABCD-1234/);
    expect(
      canvasElement.querySelectorAll(".MuiAvatar-root").length,
    ).toBeGreaterThan(0);
  },
};
...
export const TwoParticipants: Story = {
  ...
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    await canvas.findByText(/WXYZ-5678/);
  },
};
...
export const ManyParticipants: Story = {
  ...
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    await canvas.findByText(/MEGA-GROUP/);
    expect(
      canvasElement.querySelectorAll(".MuiAvatar-root").length,
    ).toBeGreaterThan(0);
  },
...
export const AllComplete: Story = {
  ...
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    await canvas.findByText(/DONE-1234/);
  },
};
