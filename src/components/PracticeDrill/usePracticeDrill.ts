...
import { calculateDrillXP, calculateAccuracyBonus } from '../../utils/practiceXPCalculator'
import { buildFeedbackData, buildFeedbackMarkdown } from './buildPracticeDrillFeedback'
...
export function usePracticeDrill(unitName: string = ''): UsePracticeDrillReturn {
  ...
  const generateDrill = useCallback(async (unitId: string, config: DrillConfig) => {
    setGenerating(true)
    setError(null)
    ...
    try {
      ...
      if (errors && errors.length > 0) {
        throw new Error(errors[0]?.message || 'Failed to generate practice drill')
      }
  ...
  const submitAnswer = useCallback((blockId: string, answer: BlockAnswer) => {
    setSession((prev) => {
      const newAnswers = { ...prev.answers, [blockId]: answer }
      const completed = Object.values(newAnswers).filter((a) => a.complete).length
    ...
    if (sessionIdRef.current) {
      ...
      client.models.PracticeSession.update({
        ...
        blocksCompleted: Object.values(session.answers).filter((a) => a.complete).length + 1,
      }).catch((err: any) => console.error('[usePracticeDrill] Save error:', err))
    }
  }, [session.answers])
  ...
  const completeDrill = useCallback(async () => {
    ...
    try {
      ...
      if (sessionIdRef.current) {
        try {
          const { data: existingGrades } = await client.models.Grade.listGradeByPracticeSessionId({
            practiceSessionID: sessionIdRef.current,
          })
          attemptNumber = (existingGrades?.length || 0) + 1
        } catch (err) {
          console.error('[usePracticeDrill] Failed to count existing attempts:', err)
        }
      }
      ...
      // Create InstructorInsight record
      try {
        await client.models.InstructorInsight.create({
          unitID: session.metadata?.unitId || '',
          studentId: '', // filled by owner auth
          practiceSessionId: sessionIdRef.current || '',
          drillType: session.metadata?.drillType || 'mixed',
          accuracy: session.accuracy,
          weakAreas: JSON.stringify(feedbackData.weakAreas),
          strongAreas: JSON.stringify(feedbackData.strongAreas),
          sourcesUsed: JSON.stringify(feedbackData.sourcesUsed),
          blockBreakdown: JSON.stringify(feedbackData.blockBreakdown),
          timestamp: new Date().toISOString(),
        })
      } catch (err) {
        console.error('[usePracticeDrill] InstructorInsight creation error:', err)
      }
      ...
      if (unitId) {
        updateUnitMemoryAndRebuild(
          ...
          console.error('[usePracticeDrill] updateUnitMemoryAndRebuild error:', err),
        )
      }
