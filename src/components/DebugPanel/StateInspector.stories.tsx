...
const mockSnapshot: StateSnapshot = {
  ...
  sessionStorage: {
    "current-session": JSON.stringify({
      id: "session-123",
  ...
  dataStore: {
    ...
    words: Array.from({ length: 128 }, (_, i) => ({
      id: `word-${i}`,
      word: `word${i}`,
    })),
  },
...
export const Default: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    expect(canvasElement.querySelector(".MuiAccordion-root")).toBeTruthy();
...
export const WithErrors: Story = {
  ...
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    await canvas.findByText(/TypeError: Cannot read property/);
  },
};
...
export const LargeDataSet: Story = {
  ...
  play: async ({ canvasElement }) => {
    expect(
      canvasElement.querySelectorAll(".MuiAccordion-root").length,
    ).toBeGreaterThan(0);
  },
};
