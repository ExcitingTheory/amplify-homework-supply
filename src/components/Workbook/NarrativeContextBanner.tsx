...
export function NarrativeContextBanner({
  unitId,
}: NarrativeContextBannerProps) {
  ...
  const linkedChallenge = activeChallenges?.find((c: any) =>
  ...
  const progressPercent =
    linkedChallenge.targetXP > 0
      ? Math.min(
          100,
          Math.round(
            (linkedChallenge.currentXP / linkedChallenge.targetXP) * 100,
          ),
        )
      : 0;
    ...
    >
      <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
        ...
        <IconButton
          ...
          aria-label={
            collapsed ? "Expand chapter context" : "Collapse chapter context"
          }
      ...
      <Collapse in={!collapsed}>
        {linkedChallenge.setting && (
          <Typography
            ...
            sx={{ mt: 1, mb: 0.5, fontStyle: "italic" }}
          >
            {linkedChallenge.setting}
          </Typography>
        )}
