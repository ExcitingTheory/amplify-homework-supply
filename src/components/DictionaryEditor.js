...
function RubyTagEditor({ inPhrase, inPronunciation, word }) {
  ...
  const _setRubyTags = (rubyTag) => {
    ...
    // updateRubyTag([...rubyTags, rubyTag]); // TODO this is where the ruby tag is sent to the parent component 
  }
  ...
  const handleSelectionEndPronunciation = (event) => {
    ...
    if (selectionStart !== null && selectionEnd !== selectionStart) {
      ...
      // _setRubyTags(`<ruby><rt>${selectedPronunciation}</rt></ruby>`);
          ...
          >
            ...
            <Button
              ...
              onClick={async () => {
                ...
                // insertWordBlock(rubyTagsString);
...
function DictionaryListItem({ entry, i, audioFiles, setPresignedUrl, refreshAudioFiles, identityId }) {
  ...
  // console.log('refreshAudioFiles', refreshAudioFiles);

  const { unit } = React.useContext(UnitContext);


  const wordAudio = entry[1]?.audio || [];
  ...
  React.useEffect(() => {
    ...
    const asyncFunc = async () => {
      ...
      await DataStore.save(Word.copyOf(entry[1], (updated) => {
  ...
  const displayOrEdit = <DisplayOrEditPhraseAndPronunciation
    phrase={phrase}
    word={entry[1]}
    pronunciation={pronunciation} />;

  const displayOrEditDefinition = <DisplayOrEditDefinition
    word={entry[1]}
  ...
  console.log('operationsClassName', operationsClassName, entry[1].phrase);

  const confirmDeleteWord = async (word) => {
    console.log('word', word);
    // TODO  show ruby characters
    // possibly nicer modal with a cancel button?
    const confirmed = window.confirm(`Are you sure you want to delete ${word.phrase}?`);

    if (!confirmed) return;
      // for each audio file, delete it. 
      // what if the user has multiple audio files?
      // what if another user has audio files for this word?
  ...
  // List phrase, definition, and pronunciation
  return <>
    <ListItem
      ...
      <ListItemText
        ...
        primary={displayOrEdit} secondary={displayOrEditDefinition} />
                  ...
                  onClick={async (e) => {
                    ...
                    try {
                      await DataStore.save(
                        Word.copyOf(entry[1], (word) => {
                          word.audio = _audioUrls;
                        })
                      )
                    } catch (error) {
...
export function DictionaryEditor() {
  ...
  console.log('DictionaryEditor.mainColor', mainColor);
  ...
  const handleCreateWord = async (event) => {
    ...
    const pronunciation = newPronunciation
    const definition = newDefinition   

    // create an audio file for the phrase and definition.
    // use the generateAudioFile mutation

    // generateAudioFile(phrase: String!, voice: String!, model: String!): String @function(name: "openai-${env}")
    ...
    try {
      ...
      const fileGeneratorDefinition = client.graphql({
        query: generateAudioFile,
        variables: {
          phrase: definition,
          voice,
        ...
        const phraseHex = createHash('sha256').update(model + voice + phrase).digest('hex');

        const definitionHex = createHash('sha256').update(model + voice + definition).digest('hex');
      ...
      const [word, file, definitionFile] = await Promise.allSettled([ dataSave, fileGenerator, fileGeneratorDefinition]);
  ...
  const onFileInputChange = async (e) => {
    ...
    // set in state for preview, then upload after word is created
    // setFiles(files);
  ...
  return (
    ...
    >
        ...
        {/* // for each word in dictionary
          ...
          // as well as the ruby and the definition */}
