    ...
    regex.test(part)
      ? React.createElement(
          "mark",
          {
            ...
            style: {
              backgroundColor:
                "var(--mui-palette-custom-searchHighlight, #ffeb3b)",
              padding: "0 2px",
            },
          },
          part,
  ...
  async loadFromIndexedDB() {
    ...
    this.loadPromise = (async () => {
      try {
        ...
        this.loaded = true;
        console.log(
          `[VectorStore] Loaded ${embeddings.length} embeddings from IndexedDB`,
        );
  ...
  async search(queryVector, filters = {}, topK = 50, queryText = "") {
    ...
    if (filters.fileId) {
      results = results.filter(
        (item) => item.metadata?.fileId === filters.fileId,
      );
    }
    if (filters.documentId) {
      results = results.filter(
        (item) =>
          item.metadata?.documentId === filters.documentId ||
          item.documentId === filters.documentId,
      );
    }
    if (filters.mimeType) {
      results = results.filter(
        (item) => item.metadata?.mimeType === filters.mimeType,
      );
    }

    // Separate items with and without embeddings
    const itemsWithEmbeddings = results.filter(
      (item) => queryVector && item.vector,
    );
    const itemsWithoutEmbeddings = results.filter(
      (item) => !(queryVector && item.vector),
    );

    console.log(
      `[VectorStore.search] Processing ${itemsWithEmbeddings.length} items with embeddings, ${itemsWithoutEmbeddings.length} without`,
    );
    ...
    if (itemsWithEmbeddings.length > 0) {
      ...
      if (itemsWithEmbeddings.length > 50) {
        console.log(
          "[VectorStore.search] Using worker for similarity calculations",
        );
        try {
          ...
          vectorResults = await this._calculateSimilaritiesMainThread(
            queryVector,
            itemsWithEmbeddings,
          );
        }
    ...
    const textResults = itemsWithoutEmbeddings.map((item) => {
      ...
      if (queryText && item.text) {
        ...
        if (textLower.includes(queryLower)) {
          ...
          similarity =
            (matchedWords.length / Math.max(queryWords.length, 1)) * 0.6;
        }
      }
    ...
    if (results.length > 100) {
      try {
        ...
        console.error(
          "[VectorStore.search] Worker sort error, falling back:",
          error,
        );
        return results
...
function MetadataEditor({ file, onUpdate, onClose }) {
  ...
  const handleSave = async () => {
    const versionCtrl =
      file._version != null ? bumpFileVersion(file.id, file._version) : null;
    try {
    ...
    <Box sx={{ p: 2, backgroundColor: "action.hover", borderRadius: 1, mt: 1 }}>
      ...
      >
        <Typography variant="h6">
          {t("fileManager2.metadataEditor.heading")}
        </Typography>
        {!editing && (
      ...
      <Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
        ...
        <Box sx={{ pt: 2, borderTop: 1, borderColor: "divider" }}>
          <Typography variant="caption" color="text.secondary" display="block">
            <strong>{t("fileManager2.metadataEditor.typeLabel")}:</strong>{" "}
            {file.mimeType}
          </Typography>
          <Typography variant="caption" color="text.secondary" display="block">
            <strong>{t("fileManager2.metadataEditor.sizeLabel")}:</strong>{" "}
            {file.size
              ? `${(file.size / 1000).toFixed(2)} KB`
              : t("fileManager2.metadataEditor.unknown")}
          </Typography>
          <Typography variant="caption" color="text.secondary" display="block">
            <strong>{t("fileManager2.metadataEditor.pathLabel")}:</strong>{" "}
            {file.path}
          </Typography>
          <Typography variant="caption" color="text.secondary" display="block">
            <strong>
              {t("fileManager2.metadataEditor.protectionLevelLabel")}:
            </strong>{" "}
            {file.level || "UNSET"}
          </Typography>
          {file.createdAt && (
            <Typography
              ...
              display="block"
            >
              <strong>{t("fileManager2.metadataEditor.createdLabel")}:</strong>{" "}
...
const ExpandedFileContent = React.memo(function ExpandedFileContent({
  file,
  parsedContent,
  search,
  editor,
  ...
  const handleInsertIntoEditor = () => {
    ...
    editor.update(() => {
      ...
      if ($isRangeSelection(selection)) {
        ...
        const fileMetadataNode = $createFileMetadataNode(
          file,
    ...
    <Box sx={{ p: 2 }}>
      ...
      {isDocument && parsedContent && (
        <Box sx={{ position: "relative", zIndex: 1, mx: -2 }}>
          ...
          >
            <Tooltip
              title={t("fileMetadataComponent.vocabulary", "Vocabulary")}
              arrow
            >
              <Tab
                icon={<MenuBookIcon />}
            ...
            <Tooltip
              title={t("fileMetadataComponent.questions", "Questions")}
              arrow
            >
              <Tab
                icon={<QuizIcon />}
            ...
            <Tooltip
              title={t("fileMetadataComponent.summaries", "Summaries")}
              arrow
            >
              <Tab
                icon={<SummarizeIcon />}
            ...
            <Tooltip
              title={t("fileMetadataComponent.objectives", "Objectives")}
              arrow
            >
              <Tab
                icon={<FlagIcon />}
            ...
            <Tooltip
              title={t("fileMetadataComponent.concepts", "Concepts")}
              arrow
            >
              <Tab
                icon={<LightbulbIcon />}
        ...
        <Box>
          ...
          >
            <Typography variant="body2" color="text.secondary">
              {t("fileMetadataComponent.type", "Type:")}
            </Typography>
            <Typography variant="body2">
              {file.mimeType || t("fileMetadataComponent.unknown", "Unknown")}
            </Typography>
            <Typography variant="body2" color="text.secondary">
              {t("fileMetadataComponent.path", "Path:")}
            </Typography>
            ...
            {file.size != null && (
              <>
                <Typography variant="body2" color="text.secondary">
                  {t("fileManager2.expandedContent.sizeLabel", "Size:")}
                </Typography>
                <Typography variant="body2">
            ...
            {file.duration != null && (
              <>
                <Typography variant="body2" color="text.secondary">
                  {t("fileManager2.expandedContent.durationLabel", "Duration:")}
                </Typography>
  ...
  React.useEffect(() => {
    const fetchImage = async () => {
      try {
        ...
        setNewDescription("");
        setPreviewMessage(
          t("fileManager2.generators.successfullyGeneratedImage"),
        );
      } catch (error) {
        console.error(error);
      }
          ...
          <Button
            ...
            onClick={async () => {
              ...
              if (path) {
                ...
                const _presignedUrl = await getCachedUrl(
                  path,
                  "protected",
                  identityId,
                );
                ...
                setPreviewMessage(
                  t("fileManager2.generators.errorGeneratingImage"),
                );
                setWorking(false);
              }
      ...
      >
        ...
        >
          <Typography
            ...
            component="h2"
          >
            {t("fileManager.textToImagePreview")}{" "}
            {working && (
              <Skeleton
                variant="rectangular"
                width={24}
...
function NewAudioFileForm({ open, toggleNewAudioFileForm }) {
  ...
  React.useEffect(() => {
    ...
    audio.addEventListener("canplaythrough", () => {
      ...
      setNewDescription("");
      setPreviewMessage(
        t("fileManager2.generators.successfullyGeneratedAudio"),
      );
    });
  }, [audioSrc]);
        ...
        >
          ...
          <Button
            ...
            onClick={async () => {
              ...
              const audioResult = await generateSpeechAction({
                phrase: newDescription,
                voice: "shimmer",
                model: "tts-1-hd",
              });
              ...
              if (path) {
                ...
                setPreviewMessage(
                  t("fileManager2.generators.errorGeneratingAudio"),
                );
                setWorking(false);
              }
      ...
      >
        ...
        >
          <Typography id="modal-modal-title" variant="h6" component="h2">
            {t("fileManager.textToSpeechPreview")}{" "}
            {working && (
              <Skeleton
                variant="rectangular"
                width={24}
...
const ProtectionLevelHeader = React.memo(function ProtectionLevelHeader({
  ...
  return (
    ...
    >
      ...
      <Typography
        variant="body2"
        sx={{ fontWeight: "bold", color: "inherit", fontSize: "0.875rem" }}
      >
        {label}
      </Typography>
      <Chip
...
const FileTypeSubheader = React.memo(function FileTypeSubheader({
  label,
  fileType,
  isExpanded,
  onToggleExpand,
  ...
  return (
    ...
    >
      ...
      <Typography
        variant="caption"
        sx={{ fontWeight: 600, fontSize: "0.8rem", lineHeight: 1 }}
      >
        {label}
      </Typography>
    </Box>
...
const FileDetailsPanel = React.memo(function FileDetailsPanel({
  file,
  documentStatus,
  editor,
}) {
  ...
  React.useEffect(() => {
    if (!file.mimeType?.startsWith("audio/") || !audioContainerRef.current)
      return;
      ...
      try {
        if (file.mimeType?.startsWith("image/")) {
          const url = await getCachedUrl(
            file.path,
            file.level?.toLowerCase() || "protected",
            file.identityId,
          );
          setImageUrl(url);
      ...
      case "re-analyze":
        try {
          if (isAnalyzableDocument(file.mimeType)) {
            console.log(
              "[FileManager2] Re-analyzing document:",
              file.name,
              "type:",
              file.mimeType,
              "documentID:",
            ...
            console.log(
              "[FileManager2] Document analysis started for:",
              file.name,
            );
            ...
            console.warn(
              "[FileManager2] Cannot re-analyze - not an analyzable document and no documentID:",
              file.name,
              file.mimeType,
            );
          }
  ...
  return (
    ...
    >
      ...
      >
        {file.mimeType?.startsWith("image/") ? (
          ...
          <Box sx={{ textAlign: "center", color: "action.disabled" }}>
            {file.mimeType === "application/pdf" && (
              <PictureAsPdfIcon sx={{ fontSize: "64px" }} />
            )}
            {file.mimeType !== "application/pdf" && (
              <DescriptionIcon sx={{ fontSize: "64px" }} />
            )}
          </Box>
        )}
      </Box>
      ...
      >
        ...
        {documentStatus && (
          <Chip label={documentStatus.status} size="small" variant="outlined" />
        )}
        <Box
          sx={{
            ml: "auto",
        ...
        >
          ...
          <Tooltip title={t("fileManager2.fileDetails.downloadTooltip")}>
            <IconButton
              ...
              aria-label={t("fileManager2.fileDetails.downloadTooltip")}
            >
          ...
          {(isAnalyzableDocument(file.mimeType) || file.documentID) && (
            <Tooltip title={t("fileManager2.fileDetails.reAnalyzeTooltip")}>
              <IconButton
                ...
                aria-label={t("fileManager2.fileDetails.reAnalyzeTooltip")}
              >
          ...
          <Tooltip title={t("fileManager2.fileDetails.deleteTooltip")}>
            <IconButton
              ...
              sx={{ color: "error.main" }}
              aria-label={t("fileManager2.fileDetails.deleteTooltip")}
            >
  ...
  function FileRowComponent({
    ...
    const handleSaveFileName = async () => {
      ...
      try {
        ...
        const finalName = newHasExtension
          ? newNameTrimmed
          : newNameTrimmed + extension;
    ...
    const handleFileNameClick = React.useCallback(
      (e) => {
        e.stopPropagation();
        setSelectedItems(new Set([file.id]));
        setIsEditing(true);
...
async function deleteFileCompletely(file) {
  ...
  try {
    ...
    if (file.documentID) {
      const { data: document } = await client.models.Document.get({
        id: file.documentID,
      });
      if (document) {
        console.log(
          "Deleting Document and all related records:",
          file.documentID,
        );
        ...
        console.log(
          `Deleted ${parsedContents?.length || 0} ParsedContent records`,
        );
        ...
        const { data: documentQuestions } =
          await client.models.DocumentQuestion.list({
            filter: { documentID: { eq: file.documentID } },
          });
        ...
        console.log(
          `Deleted ${documentQuestions?.length || 0} DocumentQuestion links`,
        );
    ...
    console.log(
      `Deleted ${fileParsedContents?.length || 0} ParsedContent records for file`,
    );
 ...
 */
function SelectedFileDetailsPanel({
  selectedItems,
  files,
  documentStatuses,
  search,
...
export default function FileManager2() {
  ...
  const [collapsedProtectionLevels, setCollapsedProtectionLevels] =
    React.useState(() => {
      if (typeof window !== "undefined") {
        const saved = localStorage.getItem(
          "fileManager2_collapsedProtectionLevels",
        );
        if (saved) {
          try {
            return new Set(JSON.parse(saved));
  ...
  const handleToggleFileTypeCollapse = (fileTypeKey) => {
    setCollapsedFileTypes((prev) => {
      ...
      if (typeof window !== "undefined") {
        localStorage.setItem(
          "fileManager2_collapsedFileTypes",
          JSON.stringify(Array.from(newSet)),
        );
      }
      return newSet;
  ...
  const handleSelectAll = () => {
    ...
    const allFileIds = new Set(
      files.filter((f) => f != null && f.id != null).map((f) => f.id),
    );
  ...
  React.useEffect(() => {
    console.log(
      "[FileManager2] Files received from context:",
      files?.length || 0,
      files,
    );
  }, [files]);
  ...
  const performSimpleTextSearch = React.useCallback(
    (query) => {
      ...
      files.forEach((file) => {
        ...
        const parsedContent =
          file.parsedContent?.[0] || file.document?.parsedContent?.[0];
        console.log(
          "[SimpleTextSearch] Parsed content for file",
          file.id,
          parsedContent,
        );
        if (parsedContent) {
          ...
          (parsedContent.questions || []).forEach((q) => {
            const questionText =
              `${q.question} ${q.answer || ""}`.toLowerCase();
            searchTerms.forEach((term) => {
  ...
  const filteredFiles = React.useMemo(() => {
    ...
    if (searchMode === "hybrid") {
      ...
      const keywordMatches = new Set(
        validFiles.filter(matchesKeyword).map((f) => f.id),
      );
  ...
  const fileListItems = React.useMemo(() => {
    ...
    Object.entries(organizedFiles).forEach(([protectionLevel, filesByType]) => {
      ...
      items.push({
        ...
        onToggleExpand: () =>
          handleToggleProtectionLevelCollapse(protectionLevel),
      });
      ...
      ["images", "audio", "documents", "video", "scripts", "other"].forEach(
        (fileType) => {
  ...
  React.useEffect(() => {
    const asyncFunc = async () => {
      ...
      console.log(
        "[FileManager2] Upload useEffect triggered. filesToUpload:",
      ...
      if (filesToUpload.length === 0 || uploadInProgressRef.current) {
        console.log(
          "[FileManager2] Skipping upload - no files or already in progress",
        );
        return;
      }
      ...
      console.log(
        "[FileManager2] About to start upload. filesToUpload:",
        filesToUpload.length,
        filesToUpload,
      );

      const fileKeys = await Promise.allSettled(
        filesToUpload.map(async (fileInput, mapIndex) => {
          const { file, index } = fileInput;
          const progressIndex = index !== undefined ? index : mapIndex;

          console.log(
            "[FileManager2] Uploading file:",
            file.name,
            "identityId:",
            identityId,
            "unitId:",
          ...
          try {
              ...
              (loaded, total) => {
                setFileOperations((prev) => {
                  ...
                  if (newFileOperations[progressIndex]) {
                    newFileOperations[progressIndex].progress =
            ...
            if (
              ...
              console.log(
                "Auto-analyzing and generating embeddings for document:",
                result.fileModel.id,
                "type:",
                file.type,
              );
      ...
      console.log(
        `[FileManager2] Upload complete. ${successes.length} succeeded, ${failures.length} failed`,
      );
  ...
  const handleDrop = async (event) => {
    ...
    const _fileOperations = _files.map((f) => ({
      name: f.name,
      progress: "0%",
    }));
  ...
  try {
          ...
          >
            ...
            >
              ...
              >
                ...
                >
                  ...
                  {t("fileManager2.contextMenu.deleteSelected", {
                    count: selectedItems.size,
                  })}
                </MenuItem>
                      ...
                      if (file) {
                        try {
                          if (isAnalyzableDocument(file.mimeType)) {
                            ...
                            await analyzePDF(file.id);
                            console.log(
                              "[FileManager2] Generating embeddings for:",
                              file.name,
                            );
                            await generateEmbeddings(file.id);
                          } else if (file.documentID) {
                          ...
                          console.error(
                            "[FileManager2] Error re-analyzing document:",
                            file.name,
                            error,
                          );
                        }
                ...
                >
                  ...
                  {t("fileManager2.contextMenu.reAnalyzeSelected", {
                    count: selectedItems.size,
                  })}
                </MenuItem>
              </Menu>
            ...
            >
              ...
              {!files?.length ? (
                ...
                >
                  <Typography>
                    {t("fileManager2.mainView.noFilesMessage")}
                  </Typography>
                </Box>
              ) : (
                <>
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  console.log(
                    "[FileManager2] Files selected:",
                    files.length,
                    files.map((f) => f.name),
                  );
                  setFilesToUpload(
                    files.map((f, index) => ({ file: f, index })),
                  );
                  setFileOperations(
            ...
            <RecordingStudio3Modal
              ...
              title={
                unit?.name
                  ? `${t("fileManager2.toolbar.recordConversation", "Record Conversation")}: ${unit.name}`
                  : t(
                      "fileManager2.toolbar.recordConversation",
                      "Record Conversation",
                    )
    ...
    return (
      <Box>{t("fileManager.errorRendering", { message: err?.message })}</Box>
    );
  }
}
