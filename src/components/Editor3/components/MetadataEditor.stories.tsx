...
// Shared smoke: the Lexical-powered name/description fields render an editable surface
const mdPlay = async ({ canvasElement }: any) => {
  await waitFor(() =>
    expect(canvasElement.querySelector("[contenteditable]")).not.toBeNull(),
  );
};
...
const createMockFile = (overrides = {}) => ({
  ...
  description:
    "Comprehensive lesson on photosynthesis in Japanese with diagrams and exercises",
  prompt:
    "Create a detailed educational lesson about photosynthesis for Japanese language learners at intermediate level",
  model: "gpt-4",
  variant: "turbo",
...
export const LargeMetadata: Story = {
  ...
  args: {
    file: createMockFile({
      ...
      description:
        "This is a comprehensive educational resource covering all aspects of Japanese grammar, vocabulary, and usage patterns. It includes detailed explanations, numerous examples, practice exercises, and cultural notes. The document is designed for intermediate to advanced learners and serves as both a learning tool and a reference guide for daily use. It covers particles, verb conjugations, honorifics, keigo, and much more in extensive detail with hundreds of example sentences.",
      prompt:
...
export const AIGeneratedImage: Story = {
  ...
  args: {
    file: createMockFile({
      ...
      description:
        "Artistic illustration of a traditional Japanese garden with koi pond",
      prompt:
        "A serene traditional Japanese garden with a koi pond, stone lanterns, cherry blossom trees, and a red wooden bridge. Watercolor style, soft colors, peaceful atmosphere.",
      model: "dall-e-3",
      variant: "hd",
      mimeType: "image/png",
      size: 2458624,
...
export const TTSAudioFile: Story = {
  ...
  args: {
    file: createMockFile({
      ...
      prompt:
        "Generate clear pronunciation audio for the following Japanese words: こんにちは、ありがとう、さようなら、おはよう、こんばんは",
      model: "tts-1",
      variant: "hd",
      mimeType: "audio/mpeg",
      size: 345678,
