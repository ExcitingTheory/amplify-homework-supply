...
export default function AudioAutoSubmitWrapper({ children, gracePeriod = 10 }) {
   ...
   */
  const wrapOnRecordingComplete = useCallback(
    (originalCallback) => {
   ...
   */
  const wrapSetFeedback = useCallback(
    (originalSetFeedback) => {
      return (data) => {
  ...
  const child = children({ wrapOnRecordingComplete, wrapSetFeedback });

  const requestSubmission = useCallback(
    (submitFn) => {
      startCountdown(submitFn, []);
    },
    [startCountdown],
  );
  ...
  return isValidElement(child)
    ? cloneElement(child, {
        submissionCountdown: countdown,
        submissionCountdownDuration: gracePeriod,
        onCancelSubmission: cancelCountdown,
        onRequestSubmission: requestSubmission,
