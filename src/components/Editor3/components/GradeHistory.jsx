...
export default function GradeHistory() {
  ...
  const averageAccuracy = hasGrades
    ? recentGrades.reduce((sum, g) => sum + (g.accuracy || 0), 0) /
      recentGrades.length
    : 0;
        ...
        <>
          ...
          <Box sx={{ mb: 3 }}>
            ...
            <Box sx={{ mb: 2 }}>
              ...
              <Typography variant="h6">{roundedAverage}%</Typography>
            </Box>
          </Box>
          ...
          <ol className="recent-grades" data-tour="correct-answers">
            {recentGrades.map((attemptGrade, index) => {
              const timeZone = Intl.DateTimeFormat().resolvedOptions().timeZone;
              const localTime = new Date(
                attemptGrade?.createdAt,
              ...
              const roundedAccuracy =
                Math.round(attemptGrade?.accuracy * 100) / 100;
              const isBest = roundedAccuracy === roundedBest;
              const completionLabel = getGradeCompletionLabel(attemptGrade);
