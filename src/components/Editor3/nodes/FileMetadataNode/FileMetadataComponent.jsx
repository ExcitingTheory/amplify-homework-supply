...
export default function FileMetadataComponent({
  ...
  const filterAndHighlight = useCallback(
    (items, textKey, searchTerm = localSearch) => {
    ...
    <Stack spacing={2}>
      ...
      <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
        ...
        <Typography variant="body2" color="text.secondary">
          ...
          {file?.size
            ...
            : t("fileMetadataComponent.unknownSize")}
        </Typography>
      </Box>
      ...
      <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
        <Typography variant="body2">
          {t("fileMetadataComponent.protectionLevel")}
        </Typography>
        <Chip
          size="small"
          variant="outlined"
      ...
      {file?.path && (
        ...
        >
          <strong>{t("fileMetadataComponent.path")}</strong>{" "}
      ...
      {file?.createdAt && (
        <Typography variant="body2" color="text.secondary">
          <strong>{t("fileMetadataComponent.created")}</strong>{" "}
      ...
      <Stack spacing={2}>
          ...
          >
            <AccordionSummary expandIcon={<ExpandMoreIcon />}>
              <Typography variant="h6">
                {t("fileMetadataComponent.vocabularyCount", {
                  count: filterAndHighlight(parsedContent.vocabulary, "term")
                    .length,
                })}
              </Typography>
            </AccordionSummary>
            <AccordionDetails>
              <List dense>
                {filterAndHighlight(parsedContent.vocabulary, "term").map(
                  (vocab, index) => (
                    <ListItem key={index} divider>
                      <ListItemText
                        primary={highlightMatches(vocab.term, localSearch)}
                        secondary={highlightMatches(
                          vocab.definition,
                          localSearch,
                        )}
                      />
        ...
        {parsedContent.summaries && parsedContent.summaries.length > 0 && (
          ...
          >
            <AccordionSummary expandIcon={<ExpandMoreIcon />}>
              <Typography variant="h6">
                {t("fileMetadataComponent.summariesCount", {
                  count: filterAndHighlight(parsedContent.summaries, "content")
                    .length,
                })}
              </Typography>
            </AccordionSummary>
            <AccordionDetails>
              <Stack spacing={2}>
                {filterAndHighlight(parsedContent.summaries, "content").map(
                  (summary, index) => (
                    <Box key={index}>
        ...
        {parsedContent.objectives && parsedContent.objectives.length > 0 && (
          ...
          >
            <AccordionSummary expandIcon={<ExpandMoreIcon />}>
              <Typography variant="h6">
                {t("fileMetadataComponent.objectivesCount", {
                  count: filterAndHighlight(
        ...
        {parsedContent.concepts && parsedContent.concepts.length > 0 && (
          ...
          >
            <AccordionSummary expandIcon={<ExpandMoreIcon />}>
              <Typography variant="h6">
                {t("fileMetadataComponent.conceptsCount", {
                  count: filterAndHighlight(parsedContent.concepts, "name")
                    .length,
                })}
              </Typography>
            </AccordionSummary>
            <AccordionDetails>
              <List dense>
                {filterAndHighlight(parsedContent.concepts, "name").map(
                  (concept, index) => (
                    <ListItem key={index} divider>
                      <ListItemText
        ...
        {parsedContent.questions && parsedContent.questions.length > 0 && (
          ...
          >
            <AccordionSummary expandIcon={<ExpandMoreIcon />}>
              <Typography variant="h6">
                {t("fileMetadataComponent.questionsCount", {
                  count: filterAndHighlight(parsedContent.questions, "question")
                    .length,
                })}
              </Typography>
            </AccordionSummary>
            <AccordionDetails>
              <List dense>
                {filterAndHighlight(parsedContent.questions, "question").map(
                  (q, index) => (
                    <ListItem key={index} divider>
                      <ListItemText
                        ...
                        secondary={
                          q.answer &&
                          highlightMatches(`Answer: ${q.answer}`, localSearch)
                        }
                      />
                    </ListItem>
                  ),
          ...
          >
            <Box sx={{ display: "flex", gap: 0, alignItems: "center" }}>
              ...
              <IconButton
                ...
                title={
                  isExpanded
                    ? t("fileMetadataComponent.collapse")
                    : t("fileMetadataComponent.expand")
                }
                onClick={(e) => {
                  e.stopPropagation();
