...
function MyOnChangePlugin({ onChange }: MyOnChangePluginProps): null {
  ...
  const throttledOnChange = useRef<
    ((editorState: EditorState, tags: Set<string>) => void) | null
  >(null);
...
export default function Editor(): React.ReactElement {
  ...
  // Auto-collapse the AppShell nav sidebar to give the editor full width
  const savedDrawerOpen = useRef(drawerOpen);
  useEffect(() => {
    if (!setDrawerOpen) return;
    savedDrawerOpen.current = drawerOpen;
    setDrawerOpen(false);
    return () => setDrawerOpen(savedDrawerOpen.current ?? false);
  ...
  const itemRefsMap = React.useRef<
    Record<string, Record<string, React.RefObject<HTMLElement>>>
  >({});

  const registerItemRef = React.useCallback(
    (type: string, id: string, ref: React.RefObject<HTMLElement>) => {
  ...
  const [actualDrawerWidthLeft, setActualDrawerWidthLeft] =
    React.useState<number>(DRAWER_WIDTH);
  const [actualDrawerWidthRight, setActualDrawerWidthRight] =
    React.useState<number>(DRAWER_WIDTH);
  ...
  const initialConfig: any = React.useMemo(
    ...
    [unit?.id],
  ); // Re-create only when unit changes (LexicalComposer reads this once on mount)
  ...
  const onChange = useCallback(
    async (
      ...
      if (
        ...
        console.log(
          "[Editor onChange] Skipping save due to tags:",
          Array.from(tags),
        );
        return;
      }
      ...
      if (!editorStateRef.current) {
        console.log(
          "[Editor onChange] Skipping save - no initial state loaded yet",
        );
        return;
      }
                ...
                <style jsx global>{`
                  ...
                  .editor:focus {
                    ...
                    box-shadow: inset 0 2px 4px
                      var(--mui-palette-action-focus, rgba(0, 0, 0, 0.1));
                  }
                `}</style>
                <AutoFocusPlugin />
                <CheckListPlugin />
                    ...
                    >
                      <div
                        ...
                        style={{
                          position: "relative",
                          minHeight:
                            "calc(100vh - var(--app-bar-height, 11rem))",
                          paddingLeft: "1.5rem",
                        }}
                      >
                        <div
                          ...
                          <RichTextPlugin
                            ...
                            placeholder={
                              <Placeholder className="editor-placeholder">
                                Start typing or use the toolbar above to add
                                content...
                              </Placeholder>
                            }
