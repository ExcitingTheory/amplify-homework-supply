...
interface WorkbookProps {
  timerGateOverlay?: React.ReactNode;
}
...
export function Workbook({
  timerGateOverlay,
}: WorkbookProps = {}): React.JSX.Element {
  const t = useTranslations("common");
  const { showUnitComplete } = React.useContext(UnitContext);
  const { appBarHeight = 48 } = useAppShell();
  const [openTab, setOpenTab] = React.useState<boolean>(false);
  const [tabValue, setTabValue] = React.useState<number>(0);
  const drawerRef = React.useRef<HTMLDivElement>(null);
  const [actualDrawerWidth, setActualDrawerWidth] =
    React.useState<number>(DRAWER_WIDTH);
  const [currentDrawerWidth, setCurrentDrawerWidth] =
    React.useState<number>(DRAWER_WIDTH);
  ...
  const scrollLocked = Boolean(timerGateOverlay) || Boolean(showUnitComplete);
