...
function convertMeaningAssociationElement(domNode) {
  ...
  const wordIDs =
    domNode.getAttribute("data-lexical-meaning-association").split(",") || [];
...
export class MeaningAssociationNode extends DecoratorNode {
  ...
  setPromptFor(promptFor) {
    const writable = this.getWritable();
    writable.__promptFor = promptFor === "word" ? "word" : "definition";
  }
  ...
  setShowAudio(showAudio) {
    this.getWritable().__showAudio = Boolean(showAudio);
  }

  setShowPronunciation(showPronunciation) {
    this.getWritable().__showPronunciation = Boolean(showPronunciation);
  }
...
export default function MeaningAssociationPlugin() {
  ...
  useEffect(() => {
    if (!editor.hasNodes([MeaningAssociationNode])) {
      throw new Error(
        "MeaningAssociationPlugin: MeaningAssociationNode not registered on editor",
      );
    }
