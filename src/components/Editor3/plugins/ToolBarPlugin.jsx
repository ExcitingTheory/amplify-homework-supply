...
import { INSERT_CUSTOM_AI_BLOCK_COMMAND } from "./CustomAIPlugin";
...
function LayoutModal({ editor }) {
  ...
  return (
    <>
      <MenuItem tabIndex={-1} onClick={handleOpen}>
        <ColumnsIcon />
          ...
          >
            ...
            >
              <Button variant="contained" color="error" onClick={handleDelete}>
                {t("toolBarPlugin.delete")}
              </Button>
              <Box sx={{ margin: "1rem" }} />
              <Button variant="contained" color="primary" onClick={handleClose}>
                {t("toolBarPlugin.cancel")}
              </Button>
            </Stack>
          </Card>
        </Modal>
...
const PreviewModal = () => {
  const t = useTranslations("editor.authoring");
  const { name, unit } = useContext(UnitContext);
  ...
  return (
    <>
      <Button color="inherit" onClick={handleOpenPreview}>
        <PreviewIcon />
      </Button>
      ...
      >
        ...
        >
          <CardContent>
            ...
            >
              {t("toolBarPlugin.closePreview", {
                name: name || t("toolBarPlugin.untitledUnit"),
              })}
            </Button>
          </CardContent>
...
const UnitTitleDescriptionEditor = ({ isScrolled = false }) => {
  const t = useTranslations("editor.authoring");
  const { name, description, saveName, saveDescription } =
...
const StatusSelect = () => {
  const t = useTranslations("editor.authoring");
  const { unit, handleStatusChange } = useContext(UnitContext);
      ...
      >
        <FormControl fullWidth>
          ...
          <Select
            ...
            onChange={handleChange}
          >
            <MenuItem value="DRAFT" tabIndex={-1}>
              <DraftIcon />
              &nbsp;
              <span className="text">{t("toolBarPlugin.draft")}</span>
            </MenuItem>

            <MenuItem value="PUBLISHED" tabIndex={-1}>
              <PublishedIcon />
              &nbsp;
              <span className="text">{t("toolBarPlugin.published")}</span>
            </MenuItem>

            <MenuItem value="ARCHIVED" tabIndex={-1}>
              <ArchivedIcon />
              &nbsp;
...
const TextAlignmentDropdown = ({ activeEditor, disabled = false, isRTL }) => {
  const t = useTranslations("editor.authoring");
  ...
  return (
    <>
      ...
      >
        ...
        >
          {isRTL ? <FormatIndentIncreaseIcon /> : <FormatIndentDecreaseIcon />}
        ...
        >
          ...
          {isRTL ? <FormatIndentDecreaseIcon /> : <FormatIndentIncreaseIcon />}
  ...
  return (
    <>
      <Button
        ...
        color="inherit"
      >
        {isBackgroundColor ? <FormatColorFillIcon /> : <FormatColorTextIcon />}
        {/* <span className="text">{title}</span> */}
      </Button>
      <Menu open={open} onClose={handleClose} anchorEl={anchorEl}>
        <ColorPicker color={color} editor={editor} onChange={onChange} />
      </Menu>
    </>
  );
};
    ...
    <>
      ...
      <Menu open={open} onClose={handleClose} anchorEl={anchorEl}>
        ...
        >
          ...
          &nbsp;
          <span className="text">
            {t("toolBarPlugin.shortAnswerVocabulary")}
          </span>
        </MenuItem>
        ...
        <MenuItem
          tabIndex={-1}
          onClick={() => {
            editor.dispatchCommand(INSERT_CUSTOM_ANSWER_BLOCK_COMMAND, []);
            handleClose();
          }}
        ...
        <MenuItem
          ...
          title="AI-Graded Exercise"
          aria-label="AI-Graded Exercise"
        >
          <FormatSizeIcon />
          &nbsp;
          <span className="text">AI-Graded Exercise</span>
        </MenuItem>
...
function FontSizeInput({ editor, value, disabled = false }) {
  ...
  const applyFontSize = useCallback(
    (newSize) => {
      const size = Math.max(
        MIN_FONT_SIZE,
        Math.min(MAX_FONT_SIZE, Number(newSize)),
      );
...
function FontDropDown({ editor, value, style, disabled = false }) {
  ...
  return (
    <>
      <style jsx global>{`
        html {
          font-family: ${notoSansJp.style.fontFamily.split(",")[0]}
            ${cormorant.style.fontFamily.split(",")[0]}
            ${inter.style.fontFamily.split(",")[0]}
            ${dmSans.style.fontFamily.split(",")[0]}
            ${oswald.style.fontFamily.split(",")[0]};
        }
      `}</style>
      ...
      <Button
        ...
        title={
          style === "font-family"
            ? t("toolBarPlugin.font")
            : t("toolBarPlugin.size")
        }
        aria-label={buttonAriaLabel}
        color="inherit"
      >
        {style === "font-family" ? <FontDownloadIcon /> : <FormatSizeIcon />}
        <span className="text">
          {style === "font-family"
            ? t("toolBarPlugin.font")
            : t("toolBarPlugin.size")}
...
const ToolBarPlugin = forwardRef(function ToolBarPlugin(
  { open, setOpen, setTabValue, isScrolled = false },
  ref,
) {
  const t = useTranslations("editor.authoring");
