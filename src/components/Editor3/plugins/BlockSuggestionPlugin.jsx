...
export default function BlockSuggestionPlugin({ useAI = false }) {
  ...
  const fetchAISuggestions = useCallback(async (forceRequest = false) => {
    ...
    try {
      ...
      while (true) {
        ...
        for (const event of events) {
          ...
          try {
            ...
            // Look for tool-result type with suggest_blocks output
            if (data.type === 'tool-result' && data.toolName === 'suggest_blocks') {
      ...
      if (toolResult?.suggestions) {
        ...
        return toolResult.suggestions.map(s => {
          const enriched = {
            ...s,
      ...
      if (error.name !== 'AbortError') {
        console.error('[SuggestBlocks] Error fetching AI suggestions:', error);
        setIsLoadingAI(false);
      }
      return null;
    }
   ...
   */
  const dispatchBlockInsert = useCallback((suggestion) => {
    ...
    editor.update(() => {
      ...
      switch (suggestion.type) {
        ...
        case 'custom-answer':
          editor.dispatchCommand(INSERT_CUSTOM_ANSWER_BLOCK_COMMAND, bd.questionIDs || null);
          break;
          
        case 'meaning-association':
          editor.dispatchCommand(INSERT_MEANING_ASSOCIATION_BLOCK_COMMAND, bd.wordIDs || null);
          break;
        ...
        case 'heading': {
          ...
          if ($isRangeSelection(selection)) {
            const anchorNode = selection.anchor.getNode();
            const block = anchorNode.getType() === 'text' ? anchorNode.getParent() : anchorNode;
            if (block) {
  ...
  }, [registerInsertCallback, dispatchBlockInsert, setSuggestions, useAI, fetchAISuggestions]);
  ...
  const renderBlockPreview = () => {
    switch (topSuggestion.type) {
      case 'heading':
        return bd.text ? (
          <span style={{ fontWeight: 700, fontSize: bd.level === 'h1' ? 18 : bd.level === 'h3' ? 13 : 15 }}>
            {bd.text}
          </span>
        ) : null;
      ...
      case 'paragraph':
        return bd.markdown ? (
          <span style={{ opacity: 0.85, maxWidth: 360, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', display: 'inline-block' }}>
            {bd.markdown.length > 80 ? bd.markdown.slice(0, 80) + '…' : bd.markdown}
          </span>
        ) : null;
      ...
      case 'custom-answer': {
        ...
        return (
          <span style={{ opacity: 0.85, fontSize: 11 }}>
            ...
            {questions[0]?.question && <> — <em>{questions[0].question.slice(0, 50)}{questions[0].question.length > 50 ? '…' : ''}</em></>}
          </span>
        );
      }
