...
import MeaningAssociationPlugin, {
...
export const EditableWithExercise = {
  render: () => (
    <EditableTemplate editorState={sampleMeaningAssociationState} />
  ),
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    ...
    await waitFor(
      () => {
        expect(
          canvas.getByText(/Vocabulary Matching Exercise/i),
        ).toBeInTheDocument();
      },
      { timeout: 5000 },
    );
    ...
    expect(
      canvas.getByText(/Match the words with their definitions/i),
    ).toBeInTheDocument();
  },
};
...
export const ReadOnlyWithExercise = {
  render: () => (
    <ReadOnlyTemplate editorState={sampleMeaningAssociationState} />
  ),
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
