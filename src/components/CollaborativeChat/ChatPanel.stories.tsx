      ...
      "topic-1": [
        {
          ...
          content:
            "Welcome to the class chat! Feel free to ask questions here.",
          mentions: [],
...
function IncomingMessageDemo() {
  const [incomingMessages, setIncomingMessages] = useState<ChatMessage[]>([]);

  const simulateIncoming = useCallback(() => {
    const senders = [
      { id: "student2", name: "Jordan Smith" },
      { id: "student3", name: "Sam Williams" },
      { id: "teacher1", name: "Ms. Tanaka" },
    ];
    const sender = senders[incomingMessages.length % senders.length];
    const texts = [
      "Has anyone started the homework yet?",
      "I found a great resource for this topic!",
      "Can we review this in class tomorrow?",
    ...
    setIncomingMessages((prev) => [
      ...prev,
      {
        ...
        content: texts[prev.length % texts.length],
        mentions: [],
        createdAt: new Date().toISOString(),
        editedAt: null,
        reactions: {},
  ...
  return (
    <Box sx={{ p: 2 }}>
      <Box sx={{ mb: 2, display: "flex", gap: 1 }}>
        ...
        >
          {incomingMessages.length} message(s) received
        </Typography>
      </Box>
      <Box
        sx={{
...
export const WithIncomingMessages: StoryObj = {
  render: () => <IncomingMessageDemo />,
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    await canvas.findByTestId("simulate-incoming");
  },
};
