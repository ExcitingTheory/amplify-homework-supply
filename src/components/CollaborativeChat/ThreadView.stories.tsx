...
const mockMessages: ChatMessage[] = [
  {
    ...
    authorName: "Alex",
    content:
      "Hey everyone! Can someone explain how を works in this sentence: 本を読みます?",
    mentions: [],
  ...
  },
  {
    ...
    content:
      "@kai Can you explain the difference between は and が with simple examples?",
    mentions: ["@kai"],
...
const meta: Meta<typeof ThreadView> = {
  ...
  decorators: [
    (Story) => (
      <div
        style={{ height: 500, border: "1px solid #e0e0e0", borderRadius: 8 }}
      >
        <Story />
      </div>
    ),
...
export const WithMessages: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await canvas.findByText(/Alex/);
    await canvas.findByText(/を marks the direct object/);
  },
};
...
export const EmptyThread: Story = {
  ...
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    expect(canvas.queryByText(/Alex/)).toBeNull();
  },
};
...
export const BotConversation: Story = {
  args: {
    messages: [
      ...
      },
      {
        ...
        authorName: "Kai",
        content:
          '食べる (taberu) means "to eat" in Japanese! It\'s a る-verb (ichidan verb), so conjugation is straightforward:\n\n- 食べます (tabemasu) - polite present\n- 食べた (tabeta) - past casual\n- 食べない (tabenai) - negative casual',
        mentions: [],
  ...
  play: async ({ canvasElement }) => {
    expect(canvasElement.textContent).toContain("食べる");
    expect(canvasElement.textContent).toContain("taberu");
  },
};
