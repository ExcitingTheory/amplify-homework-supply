...
export interface BadgeVisualConfig {
  ...
  category: 'core' | 'avatar' | 'bot-whisperer' | 'streak' | 'special'
...
export function getBadgesByCategory(category: BadgeVisualConfig['category']): Array<[string, BadgeVisualConfig]> {
  return Object.entries(BADGE_REGISTRY).filter(([, config]) => config.category === category)
}

/** Get badges filtered by rarity */
export function getBadgesByRarity(rarity: BadgeVisualConfig['rarity']): Array<[string, BadgeVisualConfig]> {
 ...
 */
export const RARITY_EFFECTS: Record<BadgeVisualConfig['rarity'], {
  borderWidth: number
  glowColor: string
  glowIntensity: number
}> = {
  common: { borderWidth: 1, glowColor: 'transparent', glowIntensity: 0 },
  uncommon: { borderWidth: 1.5, glowColor: 'rgba(76, 175, 80, 0.3)', glowIntensity: 4 },
  rare: { borderWidth: 2, glowColor: 'rgba(33, 150, 243, 0.4)', glowIntensity: 8 },
  epic: { borderWidth: 2.5, glowColor: 'rgba(156, 39, 176, 0.5)', glowIntensity: 12 },
  legendary: { borderWidth: 3, glowColor: 'rgba(255, 193, 7, 0.6)', glowIntensity: 16 },
}
