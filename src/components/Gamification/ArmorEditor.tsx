 ...
 *   - Curated heraldic palette (one of each color family) + theme primary/secondary
 *   - Metallic rim, top gloss & inner bevel finish (shared with Medallion)
...
export function renderShieldSvg(
  ...
  // crests are rendered inline on the same page.
  const uid = hashSeed(JSON.stringify(config)).toString(36);
  const clipId = `shield-clip-${uid}`;
  const rimId = `shield-rim-${uid}`;
  const glossId = `shield-gloss-${uid}`;
  const bevelId = `shield-bevel-${uid}`;
  ...
  if (chargesList.length > 0) {
    ...
    for (const ch of chargesList) {
      ...
      const pos =
        POSITIONS.find((p) => p.id === (ch.chargePosition || "center")) ||
        POSITIONS[4];
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="-4 -4 108 108" width="200" height="200">
  <defs>
    <clipPath id="${clipId}">
      <path d="${shape.path}"/>
    </clipPath>
  ...
  <path d="${shape.path}" fill="${config.fieldColor}"/>
  ${divisionFill}
  <path d="${shape.path}" fill="url(#${bevelId})" ${cp}/>
  <ellipse cx="50" cy="30" rx="40" ry="26" fill="url(#${glossId})" ${cp}/>
  ${chargeSvg}
  <path d="${shape.path}" fill="none" stroke="url(#${rimId})" stroke-width="5" stroke-linejoin="round"/>
</svg>`;
}
        ...
        <Tooltip key={shape.id} title={shape.label}>
          <Box
            ...
            sx={{
              ...
              cursor: "pointer",
              border:
                selected === shape.id ? "2px solid" : "2px solid transparent",
              borderColor:
                selected === shape.id ? "primary.main" : "transparent",
              borderRadius: 1,
              p: 0.25,
        ...
        <Tooltip key={ch.id} title={ch.label}>
          <Box
            ...
            sx={{
              ...
              cursor: "pointer",
              border:
                selected === ch.id ? "2px solid" : "2px solid transparent",
...
export function ArmorEditor({
  ...
  const addCharge = useCallback(() => {
    ...
    pushAndUpdate((prev) => ({
      ...
      config: {
        ...prev.config,
        charges: [...getCharges(prev.config), newCharge],
      },
    }));
  ...
  const updateActiveCharge = useCallback(
    (partial: Partial<ChargeConfig>) => {
      pushAndUpdate((prev) => {
          ...
          <Stack spacing={2} sx={{ flex: 1, minWidth: 0 }}>
              ...
              <Stack direction="row" flexWrap="wrap" gap={0.5} sx={{ mt: 0.5 }}>
                {palette.map((c) => (
                  <ColorSwatch
                    ...
                    onClick={() => update({ fieldColor: c.hex })}
                  />
                ))}
              </Stack>
            </Box>
                ...
                >
                  {palette.map((c) => (
                    <ColorSwatch
                      key={c.id}
                      color={c}
                      selected={config.fieldColor2 === c.hex}
                      onClick={() => update({ fieldColor2: c.hex })}
                    />
                  ))}
                </Stack>
              </Box>
                ...
                >
                  {palette.map((c) => (
                    <ColorSwatch
                      ...
                      color={c}
                      selected={
                        charges[activeChargeIndex].chargeColor === c.hex
                      }
                      onClick={() => updateActiveCharge({ chargeColor: c.hex })}
                    />
                  ))}
                </Stack>
              </Box>
            ...
            <Box>
              ...
              <ChargeSelector
                ...
                selected={
                  charges.length > 0 && activeChargeIndex < charges.length
                    ? charges[activeChargeIndex].chargeId
                    : ""
                }
                ...
                >
                  <Box>
                    ...
                    <Box sx={{ mt: 0.5 }}>
                      <PositionSelector
                        selected={
                          charges[activeChargeIndex].chargePosition || "center"
                        }
                        onSelect={(id) =>
                          updateActiveCharge({ chargePosition: id })
                        }
                      />
                    </Box>
                  </Box>
                  <Box>
                    ...
                    <Slider
                      ...
                      onChange={(_, v) =>
                        updateActiveCharge({ chargeScale: v as number })
                      }
                      min={0.5}
                      max={2}
                      step={0.25}
                  ...
                  <Box>
                    ...
                    <Slider
                      ...
                      onChange={(_, v) =>
                        updateActiveCharge({ rotation: v as number })
                      }
                      min={0}
                      max={315}
                      step={45}
                ...
                <Stack direction="row" gap={2}>
                  <Box sx={{ flex: 1 }}>
                    ...
                    <Slider
                      ...
                      onChange={(_, v) =>
                        updateActiveCharge({ offsetX: v as number })
                      }
                      min={-50}
                      max={50}
                      step={1}
                  ...
                  <Box sx={{ flex: 1 }}>
                    ...
                    <Slider
                      ...
                      onChange={(_, v) =>
                        updateActiveCharge({ offsetY: v as number })
                      }
                      min={-50}
                      max={50}
                      step={1}
