...
export const Full: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await expect(await canvas.findByText("Alpha Squad")).toBeInTheDocument();
    await expect(await canvas.findByText("Beta Team")).toBeInTheDocument();
    await expect(await canvas.findByText("Gamma Force")).toBeInTheDocument();
  },
};
...
export const NoHighlight: Story = {
  ...
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    await expect(await canvas.findByText("Alpha")).toBeInTheDocument();
    await expect(await canvas.findByText("Beta")).toBeInTheDocument();
  },
};
...
export const Empty: Story = {
  ...
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    expect(canvas.queryByText("Alpha Squad")).toBeNull();
  },
};
