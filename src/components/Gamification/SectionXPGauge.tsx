...
export function SectionXPGauge({
  ...
  const maxThresholdXP =
    projection.levelGaps[projection.levelGaps.length - 1]?.xpRequired || 1;
  const assignmentFillPercent = Math.min(
    100,
    (projection.totalAssignmentXP / maxThresholdXP) * 100,
  );
  ...
  return (
    <Box sx={{ width: "100%" }}>
        ...
        <LinearProgress
          ...
          sx={{
            ...
            "& .MuiLinearProgress-bar": {
              borderRadius: `${SEMANTIC_THEME.radius.chip}px`,
              bgcolor:
                assignmentFillPercent >= 100 ? "success.main" : "primary.main",
            },
          }}
        />
        <Typography
        ...
        {projection.levelGaps.map((gap) => (
          <Box
            ...
            sx={{
              ...
              py: 0.5,
              borderRadius: 1,
              bgcolor: gap.reachableFromAssignments
                ? "success.50"
                : "warning.50",
              border: "1px solid",
              borderColor: gap.reachableFromAssignments
                ? "success.200"
                : "warning.200",
            }}
          >
            ...
            {gap.reachableFromAssignments ? (
              ...
              <Tooltip
                title={`${gap.xpDeficit} XP deficit → ~${gap.drillSessionsNeeded} drill sessions (1/day optimal)`}
              >
                <Chip
                  icon={
                    <FitnessCenterIcon sx={{ fontSize: "0.7rem !important" }} />
                  }
      ...
      {projection.maxLevelFromAssignments.level < maxLevel ? (
        ...
        >
          Students can reach{" "}
          <strong>Level {projection.maxLevelFromAssignments.level}</strong> from
          assignments alone. Levels{" "}
          {projection.maxLevelFromAssignments.level + 1}–{maxLevel} require
          practice drills.
        </Alert>
      ) : (
        <Alert
      ...
      {showTuner && tuning && (
        <>
          ...
          <Typography
            variant="caption"
            color="text.secondary"
            sx={{ whiteSpace: "pre-line", mb: 1, display: "block" }}
          >
            {tuning.explanation}
          </Typography>
          {onApplyTuning && (
            ...
            >
              Apply Balanced Config (base={tuning.recommendedConfig.xpPerLevel},
              scale={tuning.recommendedConfig.levelScaling}×)
            </Button>
          )}
        </>
      )}
