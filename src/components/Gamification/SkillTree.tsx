...
function bfsDistances(sourceId: string, adjacency: Map<string, Set<string>>): Map<string, number> {
...
function usePhysicsPositions(
  ...
  const step = useCallback(() => {
    ...
    for (const id of ids) {
      ...
      if (Math.abs(body.vx) > VEL_THRESHOLD || Math.abs(body.vy) > VEL_THRESHOLD ||
          Math.abs(body.x - body.restX) > POS_THRESHOLD || Math.abs(body.y - body.restY) > POS_THRESHOLD) {
        hasMotion = true
      }
    }
...
export function SkillTree(props: SkillTreeProps) {
  ...
  if (loading) {
    return (
      <Box sx={{ height: rest.height || '100%', width: '100%', p: 3, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3 }}>
        {/* Top row — single root card */}
        <Box sx={{ display: 'flex', justifyContent: 'center' }}>
          <Skeleton variant="rectangular" width={240} height={100} sx={{ borderRadius: 2 }} />
        </Box>
        {/* Middle row — two cards */}
        <Box sx={{ display: 'flex', gap: 4, justifyContent: 'center' }}>
          <Skeleton variant="rectangular" width={220} height={90} sx={{ borderRadius: 2 }} />
          <Skeleton variant="rectangular" width={220} height={90} sx={{ borderRadius: 2 }} />
        </Box>
        {/* Bottom row — three cards */}
        <Box sx={{ display: 'flex', gap: 4, justifyContent: 'center' }}>
          <Skeleton variant="rectangular" width={200} height={85} sx={{ borderRadius: 2 }} />
          <Skeleton variant="rectangular" width={200} height={85} sx={{ borderRadius: 2 }} />
          <Skeleton variant="rectangular" width={200} height={85} sx={{ borderRadius: 2 }} />
        </Box>
      </Box>
