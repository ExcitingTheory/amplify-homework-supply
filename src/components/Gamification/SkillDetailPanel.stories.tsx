...
const sampleSkill: SkillNodeData = {
  ...
  description:
    "Learn common greetings and introductions in the target language.",
  status: "IN_PROGRESS",
  xpReward: 50,
...
export const InProgress: Story = {
  ...
  play: async ({ args, canvasElement }) => {
    ...
    const advanceBtn = await canvas.findByRole("button", {
      name: /Mastered|Start|Advance|Mark/i,
    });
...
export const Locked: Story = {
  ...
  play: async ({ args, canvasElement }) => {
    ...
    const advanceBtns = canvas.queryAllByRole("button", {
      name: /Mastered|Start|Advance/i,
    });
