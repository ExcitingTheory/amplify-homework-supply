...
export interface InstructorGamificationPanelProps {
  ...
  onSaveCampaign?: (
    campaign: Omit<CampaignEntry, "id"> & { id?: string },
  ) => void;
  ...
  onEditBoss?: (
    bossId: string,
    boss: BossBattleFormData | GroupChallengeEditorData,
  ) => void;
  ...
  onUpdateUnitLock?: (
    unitId: string,
    requirements: UnitLockRequirement,
  ) => void;
...
export function InstructorGamificationPanel({
  ...
  const [selectedSquadSection, setSelectedSquadSection] = useState<{
    id: string;
    name?: string;
    description?: string;
  } | null>(null);
  ...
  const badgeOptions = React.useMemo(() => {
    ...
    getAllAntiBadgeTypes().forEach((key) => {
      ...
      if (cfg)
        opts.push({ id: key, label: cfg.name || key, group: "Anti-Badges" });
    });
    ...
    <Box sx={{ maxWidth: 800 }}>
                  ...
                  secondary={
                    ...
                    >
                      ...
                      {skill.unitIds && skill.unitIds.length > 0 && (
                        <Chip
                          label={`${skill.unitIds.length} units`}
                          size="small"
                          variant="outlined"
                          sx={{ height: 18, fontSize: "0.65rem" }}
                        />
              ...
              {onGenerateCampaign && (
                <Button
                  ...
                  onClick={async () => {
                    ...
                    try {
                      const result = await onGenerateCampaign(
                        campaignTitle.trim(),
                      );
                      if (result) {
                    ...
                    <IconButton
                      ...
                      onClick={() =>
                        setExpandedCampaignId(
                          expandedCampaignId === c.id ? null : c.id,
                        )
                      }
                      aria-label={
            ...
            {squads.map((g) => (
              <ListItem key={g.id} divider sx={{ gap: 1 }}>
                ...
                <IconButton
                  edge="end"
                  size="small"
                  onClick={() => onDeleteSquad?.(g.id)}
                >
                  <DeleteIcon fontSize="small" />
                </IconButton>
              </ListItem>
            ))}
            {squads.length === 0 && <EmptyState dense title="No squads yet" />}
          </List>
        </AccordionDetails>
      </Accordion>
            ...
            {easterEggs.map((egg) => (
              <ListItem key={egg.id} divider sx={{ gap: 1 }}>
                ...
                <IconButton
                  edge="end"
                  size="small"
                  onClick={() => onDeleteEasterEgg?.(egg.id)}
                >
                  <DeleteIcon fontSize="small" />
                </IconButton>
              </ListItem>
      ...
      <Accordion defaultExpanded={false}>
        <AccordionSummary expandIcon={<ExpandMoreIcon />}>
          ...
          <Typography component="div" fontWeight={600}>
            Badges ({customBadges.length} custom, {badgeOverrides.length}{" "}
            modified)
            {badgesEnabled === false && (
              <Chip
                label="Disabled"
              ...
              <FormControlLabel
                ...
                label={
                  <Box>
                    ...
                    <Typography variant="caption" color="text.secondary">
                      When disabled, no badges (regular or anti) will be awarded
      ...
      <Accordion defaultExpanded={false}>
        ...
        <AccordionDetails>
              ...
              <FormControlLabel
                ...
                label={
                  <Box>
                    ...
                    <Typography variant="caption" color="text.secondary">
                      When disabled, anti-badges (and their debuffs) will not be
                      awarded in this section. Regular badges are unaffected by
                      this toggle.
                    </Typography>
                  </Box>
                }
          ...
          <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
            Sardonic anti-badges are awarded automatically for dubious
            achievements. Each comes with a temporary debuff — a whimsical
            penalty that makes earning them memorable. Anti-badges appear on
            student profiles alongside regular badges.
          </Typography>
          <Box
            sx={{
              display: "grid",
      ...
      <Accordion defaultExpanded={false}>
        <AccordionSummary expandIcon={<ExpandMoreIcon />}>
          ...
          <Typography fontWeight={600}>
            Boss Battles ({bossBattles.length})
          </Typography>
        </AccordionSummary>
        <AccordionDetails>
          {editingBossId ? (
            <Box>
              <Typography variant="subtitle2" sx={{ mb: 1 }}>
                Editing:{" "}
                {bossBattles.find((b) => b.id === editingBossId)?.title}
              </Typography>
              <GroupChallengeEditor
                initialData={(() => {
      ...
      {(onToggleLinearLock || onUpdateUnitLock) && (
        <Accordion defaultExpanded={false}>
          <AccordionSummary expandIcon={<ExpandMoreIcon />}>
            ...
            <Typography fontWeight={600}>
              ...
              {Object.keys(unitLockRequirements).length > 0 && (
                <Chip
                  label={`${Object.keys(unitLockRequirements).length} gated`}
                  size="small"
                  color="warning"
                  variant="outlined"
          ...
          <AccordionDetails>
            <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
              Control how students progress through units. Choose sequential
              (linear) ordering, skill-tree-based ordering, or set per-unit
              requirements like XP gates and badge requirements.
            </Typography>
            ...
            {onToggleLinearLock && (
              <Box sx={{ mb: 3 }}>
                ...
                <Stack spacing={1}>
                  <FormControlLabel
                    ...
                    label={
                      <Box>
                        ...
                        <Typography variant="caption" color="text.secondary">
                          Students must complete each unit in due-date order
                  ...
                  />
                  <FormControlLabel
                    control={<Switch checked={skills.length > 0} disabled />}
                    label={
                      <Box>
                        <Stack
                          direction="row"
            ...
            {onUpdateUnitLock && availableUnits.length > 0 && (
              <Box>
                ...
                >
                  Set XP, badge, or completion requirements that must be met
                  before a student can access each unit. These work alongside
                  linear/skill-tree ordering.
                </Typography>
                <Stack spacing={1.5}>
                  {availableUnits.map((unit) => {
                    const req = unitLockRequirements[unit.id] || {};
                    const hasReq =
                      (req.requiredXP && req.requiredXP > 0) ||
                      req.requiredBadgeId ||
                      (req.requiredModuleCompletion &&
                        req.requiredModuleCompletion > 0);
                    return (
                      <UnitLockRow
                        key={unit.id}
                        unitId={unit.id}
            ...
            {onUpdateUnitLock && availableUnits.length === 0 && (
              <Typography variant="body2" color="text.secondary" sx={{ py: 1 }}>
                No units assigned to this section. Assign units first to
            ...
            {hasReq && (
              <Stack direction="row" spacing={0.5}>
                {requirement.requiredXP ? (
                  <Chip
                    label={`${requirement.requiredXP} XP`}
                    size="small"
                    color="primary"
                    sx={{ height: 18, fontSize: "0.6rem" }}
                  />
                ...
                {requirement.requiredBadgeId ? (
                  <Chip
                    label={
                      badgeOptions?.find(
                        (b) => b.id === requirement.requiredBadgeId,
                      )?.label || requirement.requiredBadgeId
                    }
                    size="small"
                    color="secondary"
                    sx={{ height: 18, fontSize: "0.6rem" }}
                ...
                {requirement.requiredModuleCompletion ? (
                  <Chip
                    label={`${requirement.requiredModuleCompletion}% complete`}
                    size="small"
                    color="info"
                    sx={{ height: 18, fontSize: "0.6rem" }}
                  />
            ...
            <Stack spacing={1.5}>
              <Box>
                ...
                <Slider
                  ...
                  onChange={(_, val) =>
                    onUpdate({ ...requirement, requiredXP: val as number })
                  }
                  min={0}
                  max={5000}
                  step={50}
              ...
              <Autocomplete
                ...
                onChange={(_, newValue) => {
                  onUpdate({
                    ...requirement,
                    requiredBadgeId: newValue?.id || undefined,
                  });
                }}
              ...
              <Box>
                ...
                >
                  ...
                  {requirement.requiredModuleCompletion || 0}%
                </Typography>
                <Slider
                  size="small"
                  ...
                  onChange={(_, val) =>
                    onUpdate({
                      ...requirement,
                      requiredModuleCompletion: val as number,
                    })
                  }
                  min={0}
                  max={100}
...
function CopyFromSectionButton({
  sections,
  onCopy,
}: {
  sections: SectionOption[];
