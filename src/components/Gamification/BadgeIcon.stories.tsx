...
export const Locked: Story = {
  ...
  play: async ({ canvasElement }) => {
    // Locked (unearned) badge still renders its SVG (dimmed)
    expect(canvasElement.querySelector("svg")).not.toBeNull();
  },
};
...
export const AllBadges: Story = {
  ...
  play: async ({ canvasElement }) => {
    // Full registry renders many badge SVGs
...
export const Shapes: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await canvas.findByText("hexagon");
    expect(canvasElement.querySelectorAll("svg").length).toBeGreaterThan(1);
  },
};
