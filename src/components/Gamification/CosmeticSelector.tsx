...
export const DEFAULT_EDITOR_THEMES: EditorTheme[] = [
  {
    ...
    name: "Default",
    minLevel: 1,
    preview: { bg: "#ffffff", text: "#333333", accent: "#1976d2" },
  },
  {
    id: "midnight",
    name: "Midnight",
    minLevel: 2,
    preview: { bg: "#1a1a2e", text: "#e0e0e0", accent: "#00bcd4" },
  },
  {
    id: "forest",
    name: "Forest",
    minLevel: 3,
    preview: { bg: "#1b2d1b", text: "#c8e6c9", accent: "#66bb6a" },
  },
  {
    id: "sunset",
    name: "Sunset",
    minLevel: 4,
    preview: { bg: "#2d1b1b", text: "#ffccbc", accent: "#ff7043" },
  },
  {
    id: "aurora",
    name: "Aurora",
    minLevel: 5,
    preview: { bg: "#0d1b2a", text: "#e0f7fa", accent: "#ab47bc" },
  },
  {
    id: "highContrast",
    name: "High Contrast",
    minLevel: 1,
    preview: { bg: "#000000", text: "#ffffff", accent: "#ffff00" },
  },
  {
    id: "custom",
    name: "Custom",
    minLevel: 2,
    preview: { bg: "#f5f5f5", text: "#333333", accent: "#ff4081" },
  },
];
...
export function CosmeticSelector({
  ...
  const levelingTierStatuses = getStyleTierStatus(level, avatarUnlockConfig);
  const robotTierStatuses = (
    ["bottts", "bottts-neutral"] as AvatarStyleTier[]
  ).map((tier) => ({
    tier,
    unlocked: robotStylesUnlocked,
    displayName: STYLE_CONFIG[tier]?.displayName ?? tier,
  }));
  const tierStatuses = [...levelingTierStatuses, ...robotTierStatuses];
      ...
      <Box>
          ...
          {tierStatuses.map(({ tier, unlocked, displayName }) => {
            ...
            return (
              <Card
                ...
                onClick={
                  unlocked ? () => onAvatarTierSelect?.(tier) : undefined
                }
              >
                <Box
        ...
        <AvatarCustomizer
          ...
          onSave={(overrides, style) =>
            onAvatarOverridesSave?.(overrides, style)
          }
          ...
          {editorThemes.map((theme) => {
            ...
            return (
              ...
              >
                ...
                <CardContent
                  sx={{ py: "4px !important", px: 1, textAlign: "center" }}
                >
                  <Typography variant="caption" noWrap>
                    {!unlocked && "🔒 "}
                    {theme.name}
