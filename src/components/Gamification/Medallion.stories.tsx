...
import { getTinctures, getMetals, TINCTURE_KEYS } from "../../themes/heraldry";
...
export const Default: Story = {
  render: (args) => {
    ...
    const metals = getMetals(theme);
    return (
      <Medallion
        {...args}
        rimMetal={metals.gold}
        fieldColor={tinctures.azure.main}
        divisionColor={tinctures.or.main}
      />
    );
  },
  args: {
          ...
          >
            ...
            {METALS.map((metal, i) => (
              <Medallion
                ...
                shape={shape}
                size={72}
                rimMetal={metals[metal]}
                fieldColor={tinctures[TINCTURE_KEYS[i]].main}
      ...
      <Box sx={{ display: "flex", gap: 2, alignItems: "center" }}>
        {[40, 64, 96, 128].map((size) => (
          <Medallion
            ...
            shape="shield"
            size={size}
            rimMetal={metals.platinum}
            fieldColor={tinctures.gules.main}
            division="chevron"
            divisionColor={tinctures.argent.main}
