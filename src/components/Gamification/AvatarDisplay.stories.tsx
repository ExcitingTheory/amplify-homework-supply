...
export const Default: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await canvas.findByRole("img", { name: "badge" });
...
export const FireBorder: Story = {
  ...
  play: async ({ canvasElement }) => {
    await within(canvasElement).findByRole("img", { name: "badge" });
  },
};
...
export const NoBadge: Story = {
  ...
  play: async ({ canvasElement }) => {
    // No badge → avatar graphic still renders
    expect(canvasElement.querySelector("svg, img")).not.toBeNull();
  },
};
...
export const AllBorderEffects: Story = {
  render: () => {
    const effects: AvatarBorderEffect[] = [
      "none",
      "solid",
      "gradient",
      "pulse",
  ...
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    // One badge span per border-effect variant (9 total)
    const badges = await canvas.findAllByRole("img", { name: "badge" });
