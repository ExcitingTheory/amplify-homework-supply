...
export const Empty: Story = {
  render: () => {
    ...
    return (
      <RedemptionConditionForm value={condition} onChange={setCondition} />
    );
  },
  play: async ({ canvasElement }) => {
    ...
    const loginStreakOption = await within(document.body).findByText(
      "Login Streak",
    );
    ...
    expect(
      Number((countInput as HTMLInputElement).value),
...
export const Prefilled: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    const select = canvas.getByRole("combobox");
    await userEvent.click(select);
    const accuracyOption = await within(document.body).findByText(
