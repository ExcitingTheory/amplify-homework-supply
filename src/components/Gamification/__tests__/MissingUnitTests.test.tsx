...
describe("LevelBadge", () => {
  ...
  it("hides progress bar when showProgress is false", () => {
    const { container } = render(
      <LevelBadge level={baseLevel} showProgress={false} />,
    );
    const progressBar = container.querySelector('[role="progressbar"]');
  ...
  it("renders small size variant", () => {
    // "small" is an app-bar pill (Box), not a MuiChip
    const { container } = render(<LevelBadge level={baseLevel} size="small" />);
    const pill = container.querySelector('[aria-label*="Lvl. 3"]');
...
describe("StreakCalendar", () => {
  ...
  it("renders nothing before client mount when no explicit month/year", () => {
    ...
    const { container } = render(<StreakCalendar activeDays={new Set()} />);
...
describe("BadgeShelf", () => {
  ...
  it("renders all badge types (earned and unearned)", () => {
    ...
    const gridItems = container.querySelectorAll(
      '[style*="cursor: pointer"], [class*="cursor"]',
    );
  ...
  it("renders empty state for earnedOnly with no badges", () => {
    const { container } = render(<BadgeShelf earnedBadges={[]} earnedOnly />);
    const grid = container.querySelector('[class*="MuiBox-root"]');
    expect(grid).not.toBeNull();
