...
export const Interaction: StoryObj<typeof ThemeMixer> = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await expect(
      canvas.getByRole("button", { name: /Mono/i }),
    ).toBeInTheDocument();
    await expect(
      canvas.getByRole("button", { name: /Analogous/i }),
    ).toBeInTheDocument();
    await expect(
      canvas.getByRole("button", { name: /Split/i }),
    ).toBeInTheDocument();
    await expect(
      canvas.getByRole("button", { name: /Compl/i }),
    ).toBeInTheDocument();
    await expect(
      canvas.getByRole("button", { name: /Triadic/i }),
    ).toBeInTheDocument();
    await expect(
      canvas.getByRole("button", { name: /Square/i }),
    ).toBeInTheDocument();

    // Verify color input labels
    await expect(
      canvas.getByRole("textbox", { name: "Primary" }),
    ).toBeInTheDocument();
    await expect(
      canvas.getByRole("textbox", { name: "Secondary" }),
    ).toBeInTheDocument();
    await expect(
      canvas.getByRole("textbox", { name: "Background" }),
    ).toBeInTheDocument();
    await expect(
      canvas.getByRole("textbox", { name: "Paper" }),
    ).toBeInTheDocument();
    await expect(
      canvas.getByRole("textbox", { name: "Accent" }),
    ).toBeInTheDocument();
    ...
    await expect(
      canvas.getByRole("button", { name: /Save Custom Theme/i }),
    ).toBeInTheDocument();
  },
};
