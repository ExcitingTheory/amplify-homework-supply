/**
 * SquadPostEditor — Lightweight Lexical editor for creating squad posts.
 *
...
const SQUAD_POST_TOOLBAR_CONFIG: FloatingToolbarConfig = {
...
export interface SquadPostEditorProps {
...
function InitialContentPlugin({ contentJson }: { contentJson?: string }) {
  ...
  React.useEffect(() => {
    ...
    try {
      ...
      console.error("[SquadPostEditor] Failed to load initial content:", err);
    }
  }, [contentJson, editor]);
...
export function SquadPostEditor({
  onPublish,
  initialTitle = "",
  initialData,
  disabled = false,
  ...
  return (
    <Card variant="outlined" sx={{ mb: 3, overflow: "visible" }}>
      <CardContent sx={{ pb: "16px !important", overflow: "visible" }}>
        <TextField
          fullWidth
          variant="standard"
        ...
        {/* Wrapper overrides DndWrapper's overflow:hidden so the floating
            selection toolbar (rendered above the editor) isn't clipped. */}
        <Box sx={{ "& > div": { overflow: "visible !important" } }}>
          <DndWrapper>
                ...
                <LexicalComposer initialConfig={initialConfig}>
                  <Box
                    ...
                    sx={{
                      ...
                      borderColor: "divider",
                      borderRadius: 1,
                      // Top margin gives the floating selection toolbar (rendered
                      // ~44px above the editor) room so it isn't hidden behind
                  ...
                  >
                    ...
                    <RichTextPlugin
                      contentEditable={
                        <ContentEditable
                          aria-label="Squad post content"
                          style={{
                            width: "100%",
                            minHeight: 120,
                            // Left gutter reserves space for the block-inserter "+" button
                            padding:
                              "0.75rem 0.75rem 0.75rem calc(0.75rem + 36px)",
                            outline: "none",
                          }}
                        />
                      }
...
export default SquadPostEditor;
