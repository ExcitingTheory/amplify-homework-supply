...
export const Default: Story = {
  ...
  play: async ({ canvasElement, args }) => {
    ...
    const editField = await canvas.findByRole("textbox", {
      name: /Grade override/i,
    });
...
export const StudentView: Story = {
  ...
  play: async ({ canvasElement, args }) => {
    ...
    expect(
      canvas.queryByRole("textbox", { name: /Grade override/i }),
    ).toBeNull();
  },
};
    ...
    return (
      <Box>
        <Typography variant="subtitle2" color="text.secondary" sx={{ mb: 1 }}>
          Click the pencil icon to edit grades inline. Undo/redo (Cmd+Z) is
