...
export async function getRecord<T>(store: string, key: IDBValidKey): Promise<T | undefined> {
  const db = await getDB();
...
export async function deleteRecord(store: string, key: IDBValidKey): Promise<void> {
  const db = await getDB();
...
export async function getCachedUnit(unitId: string): Promise<CachedUnit | undefined> {
...
export async function getCachedWordsForUnit(unitId: string): Promise<CachedWord[]> {
...
export async function getCachedQuestionsForUnit(unitId: string): Promise<CachedQuestion[]> {
...
export async function getCachedFilesForUnit(unitId: string): Promise<CachedFile[]> {
  return getRecordsByIndex<CachedFile>('files', 'unitId', unitId);
}

export async function getCachedFileBlob(fileId: string): Promise<Blob | undefined> {
...
export async function getCachedGrade(gradeId: string): Promise<CachedGrade | undefined> {
...
export async function cacheStudentMemory(memory: CachedStudentMemory): Promise<void> {
  await putRecord('studentMemory', { ...memory, cachedAt: Date.now() });
}

export async function getCachedStudentMemory(id: string): Promise<CachedStudentMemory | undefined> {
...
export async function getPrefetchStatus(unitId: string): Promise<PrefetchStatus | undefined> {
