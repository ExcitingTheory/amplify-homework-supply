...
import type {
  SkillNodeData,
  SkillStatus,
} from "../components/Gamification/SkillTree";
...
import { trackBadgeEarned, trackLevelUp } from "../utils/analytics";
import {
  gamificationReducer,
  initialState,
  actionTypes,
...
export function GamificationProvider({
  ...
  // The safety-timeout below fires once, 5s after mount, and must read
  // whatever `state` is *at that time* — not the `state` captured when the
  // effect first ran (which would always be the all-loading initial state).
  ...
  const stableModuleNames = useMemo(
    () => moduleNames,
    [JSON.stringify(moduleNames)],
  );

  const stableEarnedBadgeTypes = useMemo(
    () => earnedBadgeTypes,
    [[...earnedBadgeTypes].sort().join(",")],
  );

  const stableModuleCompletionMap = useMemo(
    () => moduleCompletionMap,
  ...
  const setSelectedSkillId = useCallback(
    (id: string | null) =>
      dispatch({ type: actionTypes.SET_SELECTED_SKILL_ID, payload: id }),
    [],
  );
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      // Read via ref, not the `state` closed over when this effect ran (mount) —
      // otherwise this always sees the initial all-loading state and fires
      // every time regardless of whether everything actually resolved sooner.
      ...
      const stillLoading =
        ...
        s.challengesLoading ||
        s.squadsLoading ||
        s.membershipsLoading ||
        s.skillsLoading ||
        s.skillProgressLoading ||
        s.locksLoading ||
        s.xpLoading;
      if (!stillLoading) return;
      console.warn(
        "[GamificationContext] Safety timeout: forcing loading states to false",
      );
      ...
      dispatch({
        type: actionTypes.SET_SKILL_PROGRESS_LOADING,
        payload: false,
      });
  ...
  useEffect(() => {
    ...
    const processProfile = (data: any[]) => {
      ...
      // Clear loading on every response — previously only cleared when no
      // profile existed, so a real profile left these stuck true until the
      // 5s safety timeout forced them false.
      dispatch({ type: actionTypes.SET_PROGRESS_LOADING, payload: false });
      dispatch({
        type: actionTypes.SET_SKILL_PROGRESS_LOADING,
        payload: false,
      ...
      if (
        profile._version != null &&
        !(profile._version > profileVersionRef.current)
      ) {
        return;
      }
      ...
      dispatch({
        type: actionTypes.SET_RAW_MODULES,
        payload: profile.moduleProgress || [],
      });

      // Badges (was StudentBadge)
      dispatch({
        type: actionTypes.SET_RAW_BADGES,
        payload: profile.badges || [],
      });

      // Active debuffs from anti-badges
      dispatch({
        type: actionTypes.SET_RAW_DEBUFFS,
        payload: profile.activeDebuffs || [],
      });

      // Personal bests (was StudentPersonalBest)
      dispatch({
        type: actionTypes.SET_RAW_PERSONAL_BESTS,
        payload: (profile.personalBests || []).map((pb: any) => ({
          ...pb,
          id: pb.unitID,
        })),
      });
      ...
      dispatch({
        ...
        payload: (profile.skillProgress || []).map((sp: any) => ({
          ...sp,
          id: sp.skillId,
        })),
      });
    ...
    const subscription = client.models.StudentProfile.observeQuery({
      ...
      error: (err: any) => {
        ...
        if (!msg.includes("DuplicatedOperationError"))
          console.error("[GamificationContext] StudentProfile error:", err);
      },
    });
  ...
  useEffect(() => {
    ...
    const normalizedSectionIDs = Array.from(
      new Set((stableSectionIDs || []).filter(Boolean)),
    );
    const primarySectionID = sectionID || normalizedSectionIDs[0];
    if (!primarySectionID && normalizedSectionIDs.length === 0) {
      dispatch({ type: actionTypes.SET_CAMPAIGNS_LOADING, payload: false });
      dispatch({ type: actionTypes.SET_CHALLENGES_LOADING, payload: false });
      return;
    }
    ...
    const filter =
      cohortSet.size === 1
        ? { sectionID: { eq: Array.from(cohortSet)[0] } }
        : undefined;

    const processData = (data: any[]) => {
      const valid = (data || [])
        .filter((i: any) => i != null && i.id != null)
        .filter((i: any) => cohortSet.has(i.sectionID));

      // Clear loading on every response — previously only cleared in the
      // early-guard, so a real GroupChallenge response left these stuck true
      // until the 5s safety timeout forced them false.
      dispatch({ type: actionTypes.SET_CAMPAIGNS_LOADING, payload: false });
      dispatch({ type: actionTypes.SET_CHALLENGES_LOADING, payload: false });
      ...
      if (!hasChanges && Object.keys(challengeVersionMapRef.current).length > 0)
        return;
      ...
      const orderedNarrative = [...challengeWithNarrative].sort(
        (a: any, b: any) => {
      ...
      const campaignChallenge =
        ...
        valid[0];
      dispatch({
        type: actionTypes.SET_RAW_CAMPAIGNS,
        payload: campaignChallenge ? [campaignChallenge] : [],
      });
    };

    const subscription = client.models.GroupChallenge.observeQuery(
      ...
      error: (err: any) => {
        const msg = err?.message || err?.errors?.[0]?.message || String(err);
        if (!msg.includes("DuplicatedOperationError"))
          console.error("[GamificationContext] GroupChallenge error:", err);
      },
    });

    return () => subscription.unsubscribe();
  }, [client, sectionID, stableSectionIDs]);
  ...
  useEffect(() => {
    ...
    const filter = sectionID ? { sectionID: { eq: sectionID } } : undefined;

    const processSquads = (data: any[]) => {
      const valid = (data || []).filter((i: any) => i != null && i.id != null);

      // Clear loading on every response — previously only cleared in the
      // early-guard, so a real Squad response left these stuck true until
      // the 5s safety timeout forced them false.
      dispatch({ type: actionTypes.SET_SQUADS_LOADING, payload: false });
      dispatch({ type: actionTypes.SET_MEMBERSHIPS_LOADING, payload: false });
      ...
      if (!hasChanges && Object.keys(squadVersionMapRef.current).length > 0)
        return;
      ...
      dispatch({
        type: actionTypes.SET_RAW_MEMBERSHIPS,
    ...
    const subscription = client.models.Squad.observeQuery(
      ...
      error: (err: any) => {
        const msg = err?.message || err?.errors?.[0]?.message || String(err);
        if (!msg.includes("DuplicatedOperationError"))
          console.error("[GamificationContext] Squad error:", err);
      },
    });
  ...
  useEffect(() => {
    ...
    const subscription = client.models.Skill.observeQuery(
      filter ? { filter } : undefined,
    ).subscribe({
      next: ({ items }: any) => {
        const valid = (items || []).filter(
          (i: any) => i != null && i.id != null,
        );

        // Clear loading on every response — previously only cleared in the
        // early-guard, so a real Skill response left this stuck true until
        // the 5s safety timeout forced it false.
        dispatch({ type: actionTypes.SET_SKILLS_LOADING, payload: false });
        ...
        if (!hasChanges && Object.keys(skillVersionMapRef.current).length > 0)
          return;
      ...
      error: (err: any) => {
        const msg = err?.message || err?.errors?.[0]?.message || String(err);
        if (!msg.includes("DuplicatedOperationError"))
          console.error("[GamificationContext] Skill error:", err);
      },
    });
  ...
  useEffect(() => {
    ...
    const subscription = client.models.Unit.observeQuery({
      ...
      next: ({ items }: any) => {
        const valid = (items || []).filter(
          (u: any) => u != null && u.id != null && u.deletedAt == null,
        );

        // Clear loading on every response — previously only cleared in the
        // early-guard/error path, so a real Unit response left this stuck
        // true until the 5s safety timeout forced it false.
        dispatch({ type: actionTypes.SET_LOCKS_LOADING, payload: false });
        ...
        if (
          !hasChanges &&
          Object.keys(unitLockVersionMapRef.current).length > 0
        )
          return;
        ...
        const lockData = valid
          .filter(
            (u: any) =>
              ...
              u.requiredBadgeId ||
              (u.requiredModuleCompletion != null &&
                u.requiredModuleCompletion > 0),
          )
          .map((u: any) => ({
            contentId: u.id,
      ...
      error: (err: any) => {
        const msg = err?.message || err?.errors?.[0]?.message || String(err);
        if (!msg.includes("DuplicatedOperationError"))
          console.error("[GamificationContext] Unit lock error:", err);
        dispatch({ type: actionTypes.SET_LOCKS_LOADING, payload: false });
      },
    });
  ...
  useEffect(() => {
    ...
    const subscription =
      client.models.PlatformSettings.observeQuery().subscribe({
        next: ({ items }: any) => {
          const valid = (items || []).filter(
  ...
  useEffect(() => {
    if (assignmentsProp !== undefined) {
      dispatch({
        type: actionTypes.SET_RAW_ASSIGNMENTS,
  ...
  useEffect(() => {
    const hasLinearLock = (sectionsProp || []).some(
      (s: any) => s?.linearLockEnabled,
    );
    if (
      !hasLinearLock ||
      !client?.models?.Grade ||
      !studentId ||
      gradesFetchedRef.current
    ) {
    ...
    client.models.Grade.list()
      ...
      .catch((err: any) => {
        console.error(
          "[GamificationContext] Grade fetch for linear lock error:",
          err,
        );
        dispatch({ type: actionTypes.SET_GRADES_LOADING, payload: false });
      });
  ...
  useEffect(() => {
    ...
    const processXPLogs = (data: any[]) => {
      ...
      // Clear loading on every response — previously only cleared in the
      // early-guard, so a real StudentXPLog response left this stuck true
      // until the 5s safety timeout forced it false.
      dispatch({ type: actionTypes.SET_XP_LOADING, payload: false });
      ...
      if (!hasChanges && Object.keys(xpLogVersionMapRef.current).length > 0)
        return;
      ...
      if (!xpInitialLoadRef.current) {
        const newEntries = valid.filter(
          (item: any) => !knownXpIdsRef.current.has(item.id),
        );
    ...
    const subscription = client.models.StudentXPLog.observeQuery({
      ...
      error: (err: any) => {
        const msg = err?.message || err?.errors?.[0]?.message || String(err);
        if (!msg.includes("DuplicatedOperationError"))
          console.error("[GamificationContext] XPLog error:", err);
      },
    });
  ...
  const dbLevelThresholds = useMemo<
    { level: number; xpRequired: number; title?: string }[] | null
  >(() => {
    if (!globalSettings?.levelThresholds) return null;
    try {
      const parsed =
        typeof globalSettings.levelThresholds === "string"
  ...
  // Section-scoped XP: only XP logs matching the current sectionID
  const computedSectionXP = useMemo(() => {
    if (!sectionID) return 0;
    const sectionLogs = state.xpLogs.filter(
      (log: any) => log.sectionID === sectionID,
    );
    return calculateMultipliedXP(sectionLogs, xpMultipliers);
  }, [state.xpLogs, sectionID, xpMultipliers]);
  ...
  const avatarUnlockConfig = useMemo<AvatarUnlockConfig | null>(() => {
    ...
    try {
      const parsed =
        typeof globalSettings.avatarUnlockConfig === "string"
  ...
  // Emit analytics when the student levels up (baseline is set on first render)
  ...
  useEffect(() => {
    ...
    if (current > prevLevelRef.current) {
      trackLevelUp(current, computedTotalXP);
    }
    prevLevelRef.current = current;
  }, [level?.level, computedTotalXP]);

  // Emit analytics when a new (non-anti) badge is earned
  const trackedBadgesRef = useRef<Set<string> | null>(null);
  useEffect(() => {
    const keys = new Set(
      badges
        .filter((b) => !b.isAnti)
        .map((b) => `${b.badgeType}:${b.sourceId ?? ""}`),
    );
    if (trackedBadgesRef.current == null) {
      trackedBadgesRef.current = keys;
      return;
    }
    keys.forEach((key) => {
      if (!trackedBadgesRef.current?.has(key)) {
        trackBadgeEarned(key.split(":")[0]);
      }
    });
    trackedBadgesRef.current = keys;
  }, [badges]);
  ...
  const activeDebuffs = useMemo<ActiveDebuff[]>(() => {
    try {
      const raw =
        typeof state.rawDebuffs === "string"
      ...
      const info: GroupChallengeInfo = {
        ...
        progressPercent:
          ch.targetXP > 0
            ? Math.min(
                100,
                Math.round(((ch.currentXP || 0) / ch.targetXP) * 100),
              )
            : 0,
      };
  ...
  useEffect(() => {
    ...
    for (const ch of completedChallenges) {
      if (!prevIds.has(ch.id)) {
        ...
        const allSorted = [...activeChallenges, ...completedChallenges].sort(
          (a, b) => (a.chapterOrder || 0) - (b.chapterOrder || 0),
        );
        const idx = allSorted.findIndex((c) => c.id === ch.id);
        const next =
          idx >= 0 && idx < allSorted.length - 1 ? allSorted[idx + 1] : null;
      ...
      state.rawSquads
        .map((g: any) => ({
          ...
          members: (g.members || []).map((m: any) => ({
            ...
            avatarOverrides:
              typeof m.avatarOverrides === "string"
  ...
  const locks = useMemo<LockStatus[]>(() => {
    return state.rawLocks.map((lock: any) => {
      ...
      if (
        lock.requiredModuleCompletion != null &&
        lock.requiredModuleCompletion > 0
      ) {
  ...
  const linearLocks = useMemo<LockStatus[]>(() => {
    ...
    const completedUnitIds = new Set(
      state.rawGrades.filter((g: any) => g.complete).map((g: any) => g.unitID),
    );
    ...
    for (const section of linearSections) {
      ...
      if (sectionChallenges.length >= 2) {
        ...
        for (let i = 1; i < sectionChallenges.length; i++) {
          ...
          const prevComplete =
            !prevChallenge.active ||
            (prevChallenge.currentXP || 0) >= prevChallenge.targetXP;
  ...
  return (
    <GamificationContext.Provider value={contextValue}>
      ...
      <ChapterUnlockCelebration
        ...
        onClose={() =>
          setChapterCelebration((prev) => ({ ...prev, open: false }))
        }
      />
