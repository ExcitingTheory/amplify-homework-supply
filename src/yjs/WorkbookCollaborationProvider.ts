...
export interface WorkbookCollaborationConfig extends Omit<
  YjsProviderConfig,
  "docName"
> {
...
export class WorkbookCollaborationProvider extends YjsDocProvider {
   ...
   */
  loadFromGradeData(
    gradeDataJson: string | Record<string, unknown> | null,
  ): void {
    if (!gradeDataJson) return;

    try {
      const gradeData =
        typeof gradeDataJson === "string"
          ? JSON.parse(gradeDataJson)
          : gradeDataJson;
      ...
      console.log(
        `[WorkbookCollaboration] Loaded ${Object.keys(gradeData).length} blocks from Grade.data`,
      );
    } catch (error) {
  ...
  recordHistory(
    ...
    if (
      historyArray.length > WorkbookCollaborationProvider.MAX_HISTORY_ENTRIES
    ) {
      const excess =
        historyArray.length - WorkbookCollaborationProvider.MAX_HISTORY_ENTRIES;
  ...
  getBlockHistory(blockId: string): HistoryEntry[] {
    const historyArray = this.getDoc().getArray<HistoryEntry>(
      `history-${blockId}`,
    );
