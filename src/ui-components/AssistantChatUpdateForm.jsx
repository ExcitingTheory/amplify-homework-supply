...
export default function AssistantChatUpdateForm(props) {
  const {
    id: idProp,
    assistantChat: assistantChatModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = assistantChatRecord
      ? { ...initialValues, ...assistantChatRecord }
      : initialValues;
  ...
  const [assistantChatRecord, setAssistantChatRecord] = React.useState(
    assistantChatModelProp
  );
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(AssistantChat, idProp)
        : assistantChatModelProp;
      setAssistantChatRecord(record);
    };
    queryData();
  }, [idProp, assistantChatModelProp]);
  React.useEffect(resetStateValues, [assistantChatRecord]);
  const validations = {
    owner: [],
    model: [],
    threadInstructions: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            AssistantChat.copyOf(assistantChatRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "AssistantChatUpdateForm")}
      {...rest}
    >
      <TextField
        label="Owner"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || assistantChatModelProp)}
