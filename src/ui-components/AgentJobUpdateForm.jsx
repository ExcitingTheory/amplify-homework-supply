...
export default function AgentJobUpdateForm(props) {
  const {
    id: idProp,
    agentJob: agentJobModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = agentJobRecord
      ? { ...initialValues, ...agentJobRecord }
      : initialValues;
    setOwner(cleanValues.owner);
    setIdentityId(cleanValues.identityId);
    setType(cleanValues.type);
    setStatus(cleanValues.status);
    setResponseId(cleanValues.responseId);
    setWebhookData(
      typeof cleanValues.webhookData === "string" ||
        cleanValues.webhookData === null
        ? cleanValues.webhookData
        : JSON.stringify(cleanValues.webhookData)
    );
    setError(
      typeof cleanValues.error === "string" || cleanValues.error === null
        ? cleanValues.error
        : JSON.stringify(cleanValues.error)
    );
    setStartedAt(cleanValues.startedAt);
    setCompletedAt(cleanValues.completedAt);
    setModelUsed(cleanValues.modelUsed);
    setTokensUsed(cleanValues.tokensUsed);
    setEstimatedCost(cleanValues.estimatedCost);
    setRetryCount(cleanValues.retryCount);
    setMetadata(
  ...
  const [agentJobRecord, setAgentJobRecord] = React.useState(agentJobModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(AgentJob, idProp)
        : agentJobModelProp;
      setAgentJobRecord(record);
    };
    queryData();
  }, [idProp, agentJobModelProp]);
  React.useEffect(resetStateValues, [agentJobRecord]);
  const validations = {
    owner: [],
    identityId: [],
    type: [{ type: "Required" }],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            AgentJob.copyOf(agentJobRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "AgentJobUpdateForm")}
      {...rest}
    >
      <TextField
        label="Owner"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || agentJobModelProp)}
