...
const SHORTCUTS: Shortcut[] = [
  // Formatting - OS-standard shortcuts use Meta (Cmd on Mac, Ctrl on Windows)
  {
    ...
    keys: ["Meta", "b"],
    category: "formatting",
    description: "Make text bold (Cmd/Ctrl+B)",
    achievement: "Bold Beginner",
  },
  {
    id: "italic",
    name: "Italic",
    keys: ["Meta", "i"],
    category: "formatting",
    description: "Make text italic (Cmd/Ctrl+I)",
    achievement: "Italic Expert",
  },
  {
    id: "underline",
    name: "Underline",
    keys: ["Meta", "u"],
    category: "formatting",
    description: "Underline text (Cmd/Ctrl+U)",
  },
  ...
  // App-specific shortcuts use Control on ALL platforms (including Mac!)
  {
    ...
    category: "formatting",
    description: "Strike through text (Ctrl+Shift+X)",
  },
  {
    id: "clear-formatting",
    ...
    category: "formatting",
    description: "Remove formatting (Ctrl+Shift+0)",
    achievement: "Format Master",
  },
  ...
  // Block Types - Use Control on ALL platforms
  {
    ...
    category: "block",
    description: "Convert to H1 (Ctrl+Shift+1)",
    achievement: "Heading Hero",
  },
  {
    id: "heading2",
    name: "Heading 2",
    ...
    category: "block",
    description: "Convert to H2 (Ctrl+Shift+2)",
  },
  {
    id: "heading3",
    name: "Heading 3",
    ...
    category: "block",
    description: "Convert to H3 (Ctrl+Shift+3)",
  },
  {
    id: "bullet-list",
    name: "Bullet List",
    ...
    category: "block",
    description: "Create bullet list (Ctrl+Shift+8)",
    achievement: "List Legend",
  },
  {
    id: "numbered-list",
    name: "Numbered List",
    ...
    category: "block",
    description: "Create numbered list (Ctrl+Shift+7)",
  },
  {
    id: "quote",
    name: "Quote",
    keys: ["Control", "'"],
    category: "block",
    description: "Create quote block (Ctrl+')",
  },
  {
    id: "code",
    name: "Code Block",
    ...
    category: "block",
    description: "Create code block (Ctrl+Shift+C)",
  ...
  // Alignment - Use Control on ALL platforms
  {
    ...
    category: "alignment",
    description: "Align text left (Ctrl+Shift+L)",
  },
  {
    id: "align-center",
    name: "Align Center",
    ...
    category: "alignment",
    description: "Align text center (Ctrl+Shift+E)",
  ...
  },
  {
    ...
    category: "alignment",
    description: "Align text right (Ctrl+Shift+R)",
  },
  {
    id: "justify",
    name: "Justify",
    ...
    category: "alignment",
    description: "Justify text (Ctrl+Shift+J)",
  },
  ...
  // Navigation - OS-standard shortcuts use Meta
  {
    ...
    keys: ["Meta", "z"],
    category: "navigation",
    description: "Undo last action (Cmd/Ctrl+Z)",
  ...
  },
  {
    ...
    keys: ["Meta", "Shift", "z"],
    category: "navigation",
    description: "Redo last action (Cmd/Ctrl+Shift+Z)",
  },
  {
    id: "select-all",
    name: "Select All",
    keys: ["Meta", "a"],
    category: "navigation",
    description: "Select all content (Cmd/Ctrl+A)",
  },
];
...
export const KeyboardShortcutTrainer: React.FC = () => {
  ...
  useEffect(() => {
    if (completedShortcuts.size > 0) {
      localStorage.setItem(
        "keyboard-trainer-completed",
        JSON.stringify([...completedShortcuts]),
      );
    }
  }, [completedShortcuts]);

  // Persist achievements to localStorage (merge with previously stored)
  useEffect(() => {
    if (achievements.length > 0) {
      try {
        const stored = localStorage.getItem("keyboard-trainer-achievements");
        const existing: string[] = stored ? JSON.parse(stored) : [];
        const merged = [...new Set([...existing, ...achievements])];
        localStorage.setItem(
          "keyboard-trainer-achievements",
  ...
  const handleShortcutComplete = useCallback(
    (shortcut: Shortcut) => {
      ...
      setTimeout(() => {
        setCompletedShortcuts((prev) => {
          ...
          if (
            shortcut.achievement &&
            !achievements.includes(shortcut.achievement)
          ) {
          ...
          const newTier = ACHIEVEMENT_TIERS.findIndex(
            (tier) => tier.count > newSet.size,
          );
          const tierIndex =
            newTier === -1
              ? ACHIEVEMENT_TIERS.length - 1
              : Math.max(0, newTier - 1);
  ...
  // Listen for onboarding reset events to clear in-memory state
  useEffect(() => {
    const emitter = getOnboardingEmitter();
    const unsubscribe = emitter.on((event) => {
      if (event.type === "persona-selected" && event.taskId === "reset") {
        setCompletedShortcuts(new Set());
        setAchievements([]);
        setCurrentTier(0);
  ...
  const categoryProgress = (category: string) => {
    ...
    const completed = categoryShortcuts.filter((s) =>
      completedShortcuts.has(s.id),
    ).length;
  ...
  return (
    <Box sx={{ p: 3, maxWidth: 1200, mx: "auto" }} data-tour="shortcuts-demo">
      {/* Header */}
      <Box sx={{ mb: 4 }}>
        <Typography
          variant="h4"
        ...
        <Typography variant="body1" color="text.secondary" paragraph>
          Practice shortcuts by pressing the key combinations below. Each
          completed shortcut unlocks achievements!
        </Typography>
      ...
      {/* Categories */}
      <Stack spacing={3}>
        {(["formatting", "block", "alignment", "navigation"] as const).map(
          (category) => (
      ...
      {achievements.length > 0 && (
        <Paper sx={{ p: 3, mt: 3, backgroundColor: "warning.light" }}>
          <Typography
            ...
            sx={{ display: "flex", alignItems: "center", gap: 1 }}
          >
            <EmojiEventsIcon /> Achievements Earned This Session
          </Typography>
      ...
      {completedShortcuts.size === SHORTCUTS.length && (
        ...
        >
          ...
          <Typography>
            You've unlocked the secret <strong>Keyboard Master</strong>{" "}
            achievement and proven your dedication to efficiency!
          </Typography>
        </Alert>
      )}
    </Box>
